...
function weakTagsForDeck(deck) {
  const hardEvents = (deck.reviewEvents ?? []).filter((event) => event.rating === "again" || event.rating === "hard");
  ...
  if (counts.size === 0) {
    for (const node of deck.graph?.nodes ?? []) {
...
export function createLearningPlan({
  ...
  const totalDue = deckSummaries.reduce((sum, item) => sum + item.summary.dueCards, 0);
  const totalNew = Math.min(
    deckSummaries.reduce((sum, item) => sum + item.summary.newCards, 0),
  ...
  for (let day = 0; day < planDays; day += 1) {
    ...
    dayPlans.push({
      ...
      focusTopics: focusDeck?.weakTags.map((item) => item.tag) ?? [],
    });
  }
  ...
  return {
    ...
    totals: {
      ...
      dueCards: totalDue,
      newCards: totalNew,
      activeVariants: deckSummaries.reduce((sum, item) => sum + item.summary.activeVariants, 0),
      days: planDays,
    },
    deckSummaries,
    days: dayPlans,
