...
import { createCommunity, copySharedDeckToLibrary, shareDeckToCommunity } from "../communityModel.ts";
import { EmptyState, OrbIcon, PageHeader, SoftPanel } from "../ui/coreUi.tsx";

export function CommunityScreen({ decks, communities, onSaveCommunity, onSaveDeck }: any) {
  ...
  function shareSelectedDeck() {
    const deck = decks.find((item: { id: any; }) => item.id === selectedDeckId);
    if (!deck) return;
  ...
  function copyDeck(sharedRef: any) {
    const sourceDeck = decks.find((deck: { id: any; }) => deck.id === sharedRef.deckId);
    if (sourceDeck) {
              ...
              {community.folders.map((folder: { id: React.Key|null|undefined; name: string|number|bigint|boolean|React.ReactElement<unknown,string|React.JSXElementConstructor<any>>|Iterable<React.ReactNode>|React.ReactPortal|Promise<string|number|bigint|boolean|React.ReactPortal|React.ReactElement<unknown,string|React.JSXElementConstructor<any>>|Iterable<React.ReactNode>|null|undefined>|null|undefined; }) => (
                <div key={folder.id} className="rounded-xl border border-[#e3e7f5] bg-[#f8f9fe] p-4">
                  ...
                  <div className="mt-3 grid gap-2">
                    {(community.sharedDecks ?? []).filter((ref: any) => ref.folderId === folder.id).map((ref: any) => (
