...
import { maturityStageLabels, ratingButtons } from "./screenConstants.ts";
...
export function StudyMode({ deck, decks = [deck].filter(Boolean), deckId = deck?.id, variantSession, mediaStore, onExit, onDeckUpdated, onReviewEvent }: any) {
  ...
  function updateVariant(action: "disable" | "flag") {
    ...
    setSessionDecks((currentDecks: any) => replaceSessionDeck(result.deck, currentDecks));
  }
              ...
              <>
                <div className="w-full">
                  ...
                  <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold text-[#66709a]">
                    ...
                    <span className="rounded-lg bg-[#f8f9fe] px-2 py-1">{current.maturity?.label ?? (maturityStageLabels as Record<string, string>)[current.maturity?.stage] ?? "Reifegrad"}</span>
