...
import { buildDeckGraph, shouldRefreshDeckGraph } from "../deckGraph.ts";
import { EmptyState, PageHeader, SoftPanel, StatTile } from "../ui/coreUi.tsx";

export function GraphScreen({ decks, onUpdateDeck }: any) {
  const [deckId, setDeckId] = React.useState(decks[0]?.id ?? "");
  const deck = decks.find((item: { id: any; }) => item.id === deckId) ?? decks[0] ?? null;
  ...
  function generateGraph() {
    ...
    onUpdateDeck(deck.id, (current: any) => ({ ...current, graph: nextGraph }));
  }
              ...
              <div className="grid gap-6 xl:grid-cols-[1fr_0.7fr]">
                <div className="min-h-[28rem] rounded-2xl bg-[#f8f9fe] p-4">
                  <svg viewBox="0 0 720 420" className="h-full min-h-[28rem] w-full" role="img" aria-label="Deck Graph">
                    {graph.edges.map((edge: { from: any; to: any; id: React.Key|null|undefined; }) => {
                      const fromIndex = graph.nodes.findIndex((node: { id: any; }) => node.id === edge.from);
                      const toIndex = graph.nodes.findIndex((node: { id: any; }) => node.id === edge.to);
                    ...
                    {graph.nodes.map((node: { type: string; id: React.Key|null|undefined; label: string|any[]; }, nodeIndex: number) => {
