...
import { ColorPopover, ColorToolButton, defaultTextColors, normalizeColor, textPaletteColors, useStoredColorSlots } from "../ui/colorPicker.tsx";
...
function coreModeLabel(mode: string) {
  if (mode === "off") return "CoRe aus";
  if (mode === "manual") return "CoRe manuell";
  return "CoRe aktiv";
}

function CoreStatusBadge({ mode }: any) {
...
function isRowElement(target: EventTarget | null) {
...
export function LearnScreen({ decks, onStartDeck, onCreateDeck, initialParentDeckId = "", onDeckCreationHandled, onOpenCardCreation, onOpenDecks, onOpenDeckSettings, onMoveDeck }: any) {
  const library = createDeckLibraryModel(decks);
  const [collapsedDeckIds, setCollapsedDeckIds] = React.useState<Set<string>>(() => new Set());
  const [draggedDeckId, setDraggedDeckId] = React.useState<any>(null);
  const [dropIntent, setDropIntent] = React.useState<any>(null);
  ...
  const [openAppearanceMenu, setOpenAppearanceMenu] = React.useState<any>(null);
  ...
  const draggedDeckIdRef = React.useRef<any>(null);
  const lastDragEndAtRef = React.useRef(0);
  const deckCreatePanelRef = React.useRef<any>(null);
  ...
  React.useEffect(() => {
    ...
    function closeAppearanceMenu(event: { target: any; }) {
  ...
  function updateDeckAppearance(patch: Partial<DeckAppearance>|undefined) {
  ...
  function readDraggedDeckId(event: { dataTransfer: { getData: (arg0: string) => any; }; }) {
  ...
  function startDeckDrag(event: React.DragEvent<HTMLDivElement>, row: { id: string }) {
  ...
  function startDeckFromKeyboard(event: React.KeyboardEvent<HTMLDivElement>, deck: any) {
  ...
  function isInvalidDropTarget(row: { id: string; }, sourceDeckId: string) {
  ...
  function createRowDropIntent(event: React.DragEvent<HTMLDivElement>, row: { depth: number; id: any; parentDeckId: any; }) {
  ...
  function allowRowDrop(event: React.DragEvent<HTMLDivElement>, row: any) {
    event.preventDefault();
  ...
  function allowPanelTopLevelDrop(event: React.DragEvent<HTMLDivElement>) {
  ...
  function leaveDropTarget(event: { relatedTarget: any; currentTarget: { contains: (arg0: any) => any; }; }) {
    const nextTarget = event.relatedTarget;
    if (nextTarget && event.currentTarget.contains(nextTarget)) return;
    setDropIntent(null);
  }

  function dropDeck(event: React.DragEvent<HTMLDivElement>, intent = dropIntent) {
    event.preventDefault();
    event.stopPropagation();
    const sourceDeckId = readDraggedDeckId(event);
    ...
    <div className="grid min-w-0 gap-7">
          ...
          <label className="grid min-w-0 gap-2 text-sm font-semibold text-[#4e5b8c]">
            ...
            <div className="relative grid min-w-0 gap-2 text-sm font-semibold text-[#4e5b8c]">
              ...
              <button
                ...
                onClick={() => setOpenAppearanceMenu((current: any) => (current === "icon" ? null : "icon"))}
            ...
            <div className="relative grid min-w-0 gap-2 text-sm font-semibold text-[#4e5b8c]">
              ...
              <div className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#dfe4f5] bg-white px-2">
                <ColorToolButton
                  ...
                  onToggle={() => setOpenAppearanceMenu((current: any) => (current === "color" ? null : "color"))}
                />
              ...
              {openAppearanceMenu === "color" ? (
                <ColorPopover
                  ...
                  onApply={(color: any) => updateDeckAppearance({ iconColor: normalizeColor(color, DEFAULT_DECK_APPEARANCE.iconColor) })}
      ...
      {decks.length === 0 ? (
        ...
        <SoftPanel
          ...
          onDragLeave={leaveDropTarget}
          onDrop={(event: any) => dropDeck(event, { targetDeckId: null, parentDeckId: null, invalid: false, kind: "top" })}
        >
