...
test("renders scroll stories for active recall and spaced repetition", () => {
  ...
  assert.match(markup, /core-help-stack-layer-back z-0 h-\[22rem\][^>]*transform:translateX\(-30px\) scaleX\(0\.94\)/);
  assert.match(markup, /core-help-stack-layer-middle z-10 h-\[20\.5rem\][^>]*transform:translateX\(-12px\) scaleX\(0\.97\)/);
  assert.equal((markup.match(/bottom-10/g) ?? []).length, 3);
  assert.equal((markup.match(/origin-bottom/g) ?? []).length, 2);
  ...
  assert.match(markup, /core-help-stack-layer-back[^>]*transform:translate\(28px, -20px\) rotate\(3deg\)/);
  assert.match(markup, /core-help-stack-layer-middle[^>]*transform:translate\(-20px, -8px\) rotate\(-2\.5deg\)/);
  assert.match(markup, /class="core-help-stack-front absolute[^\"]*"/);
  ...
  assert.match(markup, /data-testid="memory-review-point-variant"[\s\S]*data-testid="memory-variant-star"[\s\S]*<span aria-hidden="true">×<\/span>/);
