...
test("renders scroll stories for active recall and spaced repetition", () => {
  ...
  assert.equal((markup.match(/core-help-example-stack relative mx-auto h-\[39rem\] w-full max-w-xl sm:h-\[30rem\]/g) ?? []).length, 2);
  assert.equal((markup.match(/core-help-stack-card[^\"]*h-\[30rem\]/g) ?? []).length, 6);
  assert.equal((markup.match(/core-help-stack-card[^\"]*sm:h-\[21rem\]/g) ?? []).length, 6);
  ...
  assert.match(markup, /class="mt-4 core-help-card-question text-core-text" data-testid="active-recall-answer"/);
  assert.equal((markup.match(/core-help-card-question/g) ?? []).length, 3);
  ...
  assert.match(markup, /Bei Spaced Repetition werden Fragen in gezielt gewählten Abständen wiederholt, um dem Vergessen entgegenzuwirken\. CoRe nutzt dafür den FSRS-Algorithmus, der anhand verschiedener Parameter berechnet, wie wahrscheinlich du dich noch an eine Karte erinnerst\./);
  assert.doesNotMatch(markup, /Scrolle durch die Schritte|Das Diagramm bleibt stehen/);
  assert.match(markup, /xl:grid-cols-\[minmax\(19rem,0\.65fr\)_minmax\(38rem,1\.35fr\)\]/);
  assert.match(markup, /class="min-w-0 self-start xl:order-2 xl:sticky xl:top-6" data-testid="spaced-repetition-diagram-column"/);
  assert.match(markup, /class="min-w-0 xl:order-1" data-testid="spaced-repetition-steps-column"/);
