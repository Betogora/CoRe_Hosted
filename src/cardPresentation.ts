...
function buildSrcdoc(html: string, css: string, theme: "light" | "dark"): string {
  const background = theme === "dark" ? "#17151f" : "#ffffff";
  const foreground = theme === "dark" ? "#f4f0ff" : "#211b2b";
  return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; base-uri 'none'; connect-src 'none'; font-src data: blob:; form-action 'none'; frame-src 'none'; img-src data: blob:; media-src data: blob:; object-src 'none'; script-src 'none'; style-src 'unsafe-inline'"><meta name="color-scheme" content="${theme}"><style>:root{color-scheme:${theme}}html,body{margin:0;max-width:100%;overflow-wrap:anywhere}body{box-sizing:border-box;background:${background};color:${foreground};font:inherit;padding:1rem}.core-field-separator{height:.75rem}.core-compatibility-notice{border:1px solid currentColor;border-radius:.75rem;margin-bottom:1rem;padding:.75rem}.core-fallback-field+ .core-fallback-field{margin-top:1rem}.core-fallback-field h3{font-size:.875rem;margin:0 0 .375rem;opacity:.72}img,video{height:auto;max-width:100%}${css}</style></head><body>${html}</body></html>`;
}
...
export function renderLearningItemPresentation(input: {
  ...
  const frontSide = frontAst
    ...
    : "";
  const rawBodyHtml = preservedOnly || !compiled.ast
  ...
  return {
    srcdoc: buildSrcdoc(bodyHtml, css, input.theme),
