...
export function shareDeckToCommunity(community: { id?: any; name?: string; ownerId?: string; joinMode?: string; maxMembers?: number; members?: { userId: string; role: string; displayName: string; }[]; folders?: any; sharedDecks?: any; createdAt?: string; }, deck: Deck, { folderId = null, permission = "copy", sharedBy = "local-user" }: any = {}) {
  ...
  return {
    community: {
      ...community,
      sharedDecks: [sharedRef, ...(community.sharedDecks ?? []).filter((ref: { deckId: any; }) => ref.deckId !== deck.id)],
    },
    sharedRef,
  };
}

export function copySharedDeckToLibrary(sourceDeck: Deck, { nameSuffix = "Kopie" }: any = {}) {
  ...
  return createCoreDeck({
    ...
    createdAt: copiedAt,
    updatedAt: copiedAt,
    cards: (sourceDeck.cards ?? []).map((card: { variants: any; }) => ({
      ...card,
      id: makeId("card"),
      deckId: "",
      source: "community",
      reviewState: null,
      variants: (card.variants ?? []).filter((variant: { qualityStatus: string; }) => variant.qualityStatus === "active"),
    })),
    reviewEvents: [],
    aiJobs: [],
...
export function assertCommunityPrivacyPayload(community: any) {
