...
test("deck tree keeps one visual header and all three accessibly labelled metrics in every row", () => {
  ...
  assert.match(markup, /<button[^>]*class="[^"]*-me-2[^"]*before:inset-x-1\.5 before:inset-y-1[^"]*hover:before:bg-\[var\(--core-surface-hover\)\][^"]*"[^>]*aria-label="Unterstapel von Bereich ausblenden"/);
  assert.doesNotMatch(markup, /\[&amp;&gt;svg\]:-translate-x-1| hover:bg-\[var\(--core-surface-hover\)\]/);
...
test("deck tree fills hovered rows without separating deck headers", () => {
  const boundaryDecks = [
    ...decks,
  ...
  assert.doesNotMatch(markup, /data-testid="learn-deck-row-[^"]+"[^>]*class="[^"]*border-t-2/);
