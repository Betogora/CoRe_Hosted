...
- `CardPreviewDialog` aus `src/ui/CardPreviewDialog.tsx`: gemeinsamer kontrollierter Portal-Dialog für den aktuellen ungespeicherten Kartenentwurf. Desktop verwendet eine große zentrierte Kartenbühne, Mobile den gesamten Viewport. Das Modul besitzt Fokusfalle, Scrollsperre, Escape-/Außenklick-Schließen, Fokuswiederherstellung und eine reguläre `CoreSegmentedControl`-Seitenauswahl; es rendert immer nur eine `CardPresentationSurface` und startet bei jedem Öffnen auf `Vorderseite`.
