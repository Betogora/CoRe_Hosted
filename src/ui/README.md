...
- `InPageNavigation` aus `src/ui/InPageNavigation.tsx`: gemeinsame Seiteninhaltsnavigation und zweispaltiges Inhaltslayout für globale und stapelspezifische Einstellungen. Die Komponente erhält stabile Abschnitts-IDs, deutsche Labels, Lucide-Icons und den Seiteninhalt. Ab `xl` rendert sie eine vertikale Sticky-Rail, darunter ein unter dem gemessenen App-Kopf positioniertes Sticky-Disclosure mit ein- beziehungsweise zweispaltiger Linkliste. Native Hashlinks, Scrollspy, genau ein `aria-current="location"`, Außenklick/Escape, Tastaturfokus und Reduced Motion gehören vollständig dem Modul; Statuszeilen, semantische Kartenflächen und Zähler gehören nicht zu diesem Muster.
