...
- `CardPresentationSurface` aus `src/ui/CardPresentationSurface.tsx`: gemeinsamer sicherer Kartenrenderer für Erstellungsvorschau, Kartenverwaltung und Review. Die Komponente erhält Learning Item, Variantenprojektion, Definition, Seite, Oberfläche und Theme; Template-, Sandbox- und Kompatibilitätslogik bleiben in `cardPresentation.ts`.
