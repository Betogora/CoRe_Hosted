...
- `SettingsSectionNavigation` und `SettingsCrossLinkButton` aus `src/ui/settingsNavigation.tsx`: gemeinsame responsive Bereichsnavigation für globale und stapelspezifische Einstellungen. Die Navigation erhält drei fachlich beschriftete Einträge mit Lucide-Icon, Titel, Kurzstatus und semantischem `info`-, `success`- oder `warning`-Ton; native Buttons beziehungsweise Links bleiben tastaturbedienbar und wechseln ab `md` von einer gestapelten Liste in drei Spalten. Der kompakte Querlink übernimmt Form und Chevron-Hierarchie der Dashboard-Aktion `Alle ansehen`; Ziel, Beschriftung und Navigation bleiben beim Aufrufer.
