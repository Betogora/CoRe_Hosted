...
Stapelbezogene Auswahl verwendet `DeckSelect` für genau einen Stapel und `DeckMultiSelect` ausschließlich für den Statistik-Scope. Beide Varianten teilen Overlay, Typografie, 44-px-Zeilen, Hierarchieeinrückung und eine höchstens 320 px hohe Liste mit nativer Seiten-Scrollbar. Ab fünf tatsächlich auswählbaren Stapeln erscheint eine Suche über den vollständigen Hierarchiepfad; Sonderziele wie Hauptstapel, Hauptebene oder gesamte Sammlung zählen nicht zur Schwelle und bleiben von der Suche getrennt. `DeckSelect` zeigt im geschlossenen Zustand Stapel-Icon und vollständigen Pfad und markiert die einzelne Auswahl rechts mit einem Haken. `DeckMultiSelect` verwendet links Checkboxen; durch einen gewählten Oberstapel eingeschlossene Unterstapel bleiben angehakt und deaktiviert, jedoch einzeilig. Tiefen steuern ausschließlich die Einrückung und niemals eine Gruppenfarbe.
