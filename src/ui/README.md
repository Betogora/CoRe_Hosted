...
- `StudyHeatmap` aus `src/ui/StudyHeatmap.tsx`: gemeinsame responsive Lern-Heatmap für Dashboard und Statistik mit identischer Paneloptik, Intensitätslegende, Wochenfenster-Navigation, Tastatursteuerung und CoRe-Tooltips. Die Aufrufer liefern das normalisierte dünne Heatmap-Modell sowie ihre fachliche Tagesbeschriftung.
...
- `StudySettingsOverlay` aus `src/ui/StudySettingsOverlay.tsx`: gemeinsame modale Lerneinstellungen mit den Abschnitten `Karte`, `Sitzung` und `Stapel`. Dasselbe DOM erscheint unter 768 px als Bottom Sheet und ab 768 px als zentrierter Dialog; das Modul besitzt Portal, Fokusfalle, Escape-/Außenklick-Schließen, Fokuswiederherstellung, Viewportscroll und Reduced Motion. Fachliche Werte, Kartenbearbeitung und Speichern der Stapelgrenzwerte bleiben Callbacks des Aufrufers.
