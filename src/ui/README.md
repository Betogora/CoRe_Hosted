...
- `LearningSettingsPanel` aus `src/ui/LearningSettingsPanel.tsx`: gemeinsame kompakte Projektion fachlicher Lernparameter und Presets. Neue Karten, tägliche Wiederholungen und maximales Intervall verwenden ein lokales responsives Ganzzahlfeld ohne Slider beziehungsweise Auswahl; es übernimmt gültige Werte beim Verlassen oder mit Enter und verwirft leere beziehungsweise ungültige Zwischenstände. Selbsterklärende Felder bleiben ohne wiederholenden Hinweis; Folgen, Abwägungen und Warnungen bleiben sichtbar. Tagespensum und Content-Repetition-Felder wechseln responsiv zwischen drei, zwei und einer Spalte. Der Schalter für kurze Abstände verwendet `CoreSwitch`. Der globale Aufrufer aktiviert automatisches Speichern, übergibt den bewahrten eigenen Entwurf und kann `dayStartHour` übergeben; dann erscheint ausschließlich dort die Zeile `Neuer Tag beginnt um [Zahl] Stunden nach Mitternacht` mit nativem Ganzzahlfeld von 0 bis 23 und außen stehender Einheit. Stapelspezifische Aufrufer speichern ausdrücklich und können stattdessen `variantThresholdXp` und `maxActiveVariantsPerCard` übergeben; Preset-Wechsel erhalten alle drei ergänzenden Werte.
