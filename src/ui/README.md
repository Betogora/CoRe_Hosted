...
- `StudyHeatmap` aus `src/ui/StudyHeatmap.tsx`: gemeinsame responsive Lern-Heatmap für Dashboard und Statistik mit dynamischem Streak-Titel, lokaler Segmentauswahl für die letzten sieben Tage, den Kalendermonat oder das Kalenderjahr, gleich hoher Zeitraum-Navigation, Intensitätslegende und CoRe-Tooltips. Eine native Container-Query hält den Kopf ab 34 rem Heatmapbreite einzeilig und darunter stabil zweizeilig. Woche und Monat passen sich ohne Seitenüberlauf an; das vollständige Jahresraster besitzt auf schmalen Ansichten einen eigenen horizontalen Scrollbereich. Die Aufrufer liefern das normalisierte dünne allzeitliche Heatmap-Modell sowie ihre fachliche Tagesbeschriftung.
- `DeckSummaryRow` aus `src/ui/DeckSummaryRow.tsx`: kanonischer einzeiliger Zeilenrenderer für Dashboard, Lernen und Kartenverwaltung mit Chevron-Slot, Stapel-Icon, Name ohne sichtbaren Hierarchiepfad, den disjunkten Kennzahlen `Neu`, `In Arbeit` und `Fällig`, Donut und Aktionsslot. Seine responsive Dichte blendet unter 44 rem tatsächlich verfügbarer Zeilenbreite nur die sichtbaren Kennzahllabel aus, ohne eine zweite Zeile oder ein zweites Menü zu mounten. Die Kartenverwaltung liefert direkte Werte, Dashboard und Lernen aggregieren Unterstapel; alle drei Ansichten verwenden dasselbe reduzierte Stapelmenü.
