...
- Die Hauptnavigation lautet exakt `Heute`, `Lernen`, `Erstellen`, `Karten`, `Statistik`. Ein Klick auf den Schriftzug `CoRe` im App-Kopf beziehungsweise in der Sidebar navigiert zu `Heute`.
...
- Die Kartenverwaltung bündelt die Unterüberschrift `Kartenverwaltung`, Suche und gruppierte Gesamttabelle in einem gemeinsamen Panel analog zu `Aktive Stapel`. Es gibt dort keinen Direkteinstieg zur Kartenerstellung.
...
- Der Detailbereich zeigt den typgerechten Editor sowie `Speichern`, `Vorschau`, `Kopieren` und `Löschen` primär; Varianten, Herkunft, Versionen und Restore folgen als dauerhaft sichtbare Abschnitte ohne ausklappbare Inhaltsflächen. Die frühere ausklappbare Karten-Vorschau im Detail existiert nicht mehr. Markieren und Aussetzen sind davon unabhängige, sofort gespeicherte Lernstatus-Aktionen, sodass insbesondere eine ausgesetzte Karte direkt reaktiviert werden kann, ohne ungespeicherte Inhaltsänderungen zu übernehmen.
- Der gemeinsame modale Karten-Vorschau-Dialog zeigt bei jedem Öffnen zunächst ausschließlich die Vorderseite und wechselt über eine segmentierte Seitenauswahl zur Rückseite aus Frage, Trennlinie und Antwort. Single Choice deckt nach einer Auswahl direkt auf, Multiple Choice erst über `Antwort prüfen`; die Rückkehr zur Vorderseite setzt die Auswahl zurück. Er verwendet den aktuellen ungespeicherten Entwurf, folgt dem aktiven Light-/Dark-Theme, füllt auf Mobilgeräten den Viewport und besitzt Fokusfalle, Escape-, Außenklick- und Fokuswiederherstellungsverhalten.
- Basic-Editoren lesen Frage und Antwort aus den semantisch zugeordneten Dokumentfeldern. Eine durch Anki-`FrontSide` materialisierte Frage ist deshalb kein Bestandteil des Rückseiten-Eingabefelds; Speichern erhält die rohe Antwort. Die Karten-Vorschau setzt CoRe-eigene Rückseiten aus Frage, Trennlinie und Antwort genau einmal zusammen und erhält bei importierten Anki-Templates deren `FrontSide`-Semantik. Im Review bleibt die bereits sichtbare Frage außerhalb der Antwort stehen; die Antwortprojektion wiederholt weder eine CoRe-Frage noch Anki-`FrontSide`.
...
- Reverse zeigt im normalen Review die Originalrichtung und im ausdrücklich gestarteten Variantenreview die atomar synchronisierte Rückrichtung. Rückrichtung wird für Single Choice und Multiple Choice nicht angeboten.
...
- Importierte Rohfelder bleiben in den dauerhaft sichtbaren Details read-only; Quellen, Versionen und Wiederherstellung sind ohne zusätzliche Interaktion verfügbar.
...
- In den dauerhaft sichtbaren Variantenwerkzeugen kann eine Basic-Karte unmittelbar als KI-Variante umformuliert werden. Die erzeugte Form ist kein neues Learning Item, sondern eine aktive Variante auf Level 2 mit `generationSource: "ai_generated"` und demselben Originalanker; das manuelle Front-/Back-Formular bleibt erhalten.
...
- Eine eigenständige, als vereinfacht gekennzeichnete Lernkurve führt zuerst zu einer gemeinsamen Wiederholung und fächert danach in die vier Antwortpfade `Nochmal`, `Schwer`, `Gut` und `Leicht` mit unterschiedlich langen Stabilitätsintervallen auf. Am Ende des längsten Beispielintervalls folgt eine weitere Wiederholung mit einer nahen CoRe-Variante; ein Kreis mit `…`, die Beschriftung `x. Wiederholung · Variante` und der Stern an der gestrichelten Wiederholungsmarkierung verdeutlichen, dass daraus keine garantierte Scheduler- oder Variantenschwelle entsteht. Die durch zwei diagonale Striche unterbrochene Y-Achse trägt den Hinweis `Ausschnitt 90–100 %`.
...
- App-Navigation, Querlinks, Zurück, Stapelwechsel, Abmelden, weitere Kontextaktionen und Browser-Zurück/Vorwärts werden bei ungespeicherten Einstellungen abgebrochen; die Leiste kündigt dann höflich `Zum Verlassen zuerst speichern.` an. Erfolgreiches Speichern bleibt auf der aktuellen Seite, und das gewünschte Ziel muss erneut gewählt werden. Reine Hashnavigation innerhalb der Einstellungsseite bleibt frei; Reload, Tab-Schließen und externe Navigation verwenden die native Browserwarnung. Lernprofil-CRUD, `Auf diesen Stapel anwenden`, manueller Sync, Export, Import und die Lerneinstellungen einer Kartensitzung behalten ihre unmittelbaren eigenen Aktionen und erzeugen keine Einstellungsleiste.
...
- `Öffnen` erzeugt einen automatisch bereinigbaren Cache. `Offline verfügbar machen` lädt Karten, Varianten, Darstellungsvorlagen, Quellsnapshots und Medien fortsetzbar in 50er-Paketen und setzt den Zustand erst nach Anzahl-, Revisions- und Hashprüfung auf verfügbar. Cloudänderungen markieren den Download als aktualisierungsbedürftig. `Offline-Daten entfernen` löst nur die Anheftung; Outbox, lokale Änderungen und aktive Lernkarten bleiben geschützt.
...
- Statistiken werden aus serverseitigen Aggregaten geladen und lokal gecacht; lokale noch nicht bestätigte Reviews ergänzen sie optimistisch. Ein vollständiger Export streamt online fehlende kanonische Struktur- und Reviewdaten und wird offline nur für vollständig heruntergeladene Stapel erzeugt. Vor einem APKG-Reimport werden bestehende betroffene Stapel vollständig hydriert. Das Löschen eines nicht geladenen Stapelbaums wird als accountgebundenes Serverkommando ausgeführt und offline in der Outbox vorgemerkt.
- `Karten` lädt höchstens 50 Tabellenzeilen je Seite. Eine Lernsitzung wartet nur auf den ersten benötigten Kartenkörper, füllt danach im Hintergrund einen Puffer von höchstens 50 Karten und fordert bei 25 verbleibenden Karten die nächste 50er-Seite an. Vollständige Stapel dürfen weder für App-Start noch Kartenliste oder Lernstart materialisiert werden.
