...
- Pointer-Hover füllt Stapelzeilen in Dashboard, Lernen und Kartenverwaltung neutral mit derselben dunkleren Graufläche; eine bloße interaktive Rahmen- oder Unterstreichungsfarbe erscheint dabei nicht. In allen drei Ansichten trennt eine 2-px-Linie ausschließlich zwei aufeinanderfolgende Hauptstapel-Bäume. Innerhalb eines Baums besitzen Stapelköpfe unabhängig von ihrer Hierarchieebene keine Trennlinie; die dünnen Grenzen zwischen einzelnen Kartenzeilen der Kartenverwaltung bleiben erhalten.
...
Ein Fragezeichen neben dem Theme-Schalter öffnet die direkt verlinkbare Hilfeseite `/hilfe` im normalen App-Shell-Inhaltsbereich. Ein kurzer Einstieg ordnet CoRes Ziel ein. Ein klar gestaffelter, einheitlich hellblauer Kartenstapel mit subtilen CoRe-Rahmen fragt dort, welche Grundsätze CoRe für möglichst nachhaltiges Lernen nutzt. Schlüsselwörter tragen eine pinke Textmarkierung; die gelben Pfeile verbinden `Active Recall → Smarter Recall` sowie `Spaced Repetition → Content Repetition`. Zwei anschließende Scrollgeschichten erklären erst den wechselnden Abrufreiz und danach die FSRS-Lernkurve, ohne Workspace- oder Schedulerzustand zu verändern.
