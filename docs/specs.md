...
| Auth und Cloud-Persistenz | `AuthGateScreen`, `SettingsScreen`, `accountSession`, `accountStorage`, `supabaseClient`, `cloudAuth`, `cloudRepository`, Pflichtlogin, Supabase-E-Mail/Passwort, Profil-Upsert, accountgebundener Cache, revisionsbedingter Delta-Autosave, Soft-Delete-Tombstones, einmaliger lokaler Importdialog |
...
- Supabase Browser-Client ueber `VITE_SUPABASE_URL` und `VITE_SUPABASE_PUBLISHABLE_KEY`, Pflicht-Login-Gate, echte E-Mail/Passwort-Registrierung und Anmeldung, Profil-Upsert in `public.profiles`, accountgebundener Browser-Cache, einmalige lokale Datenuebernahme und revisionsbedingter Delta-Autosave fuer Decks, Karten/Learning Items, Varianten, Review Events, Dokumente und AI Jobs. Unveränderte Rows werden nicht geschrieben, Review-Events bleiben append-only und Remote-Tombstones werden nicht reaktiviert.
...
| `src/accountStorage.js` | `createAccountStorage`, `readLegacyLocalState`, `hasPendingLocalMigration`, `markLocalMigrationHandled` | Accountgebundene Browser-Cache-Keys und einmalige lokale Datenuebernahme |
| `src/cloudRepository.js` | `replaceAccountCloudState`, `upsertAccountCloudState`, `loadAccountCloudState`, `mergeCloudSyncMetadata`, `listAccountSyncConflicts`, `resolveAccountSyncConflict`, Mapping-Helfer | Accountgefiltertes Laden, unsichtbare Soft-Delete-Tombstones, revisionsbedingte Delta-Updates, append-only Review-Events, Server-Acknowledgements und bewusster Voll-Replace fuer Legacy-Import/Test-Reset |
| `src/syncEngine.js`, `src/syncOutbox.js` | `createAccountSyncEngine`, `createSyncEngine`, `createSyncOutbox`, `enqueueMutation`, `flush`, `listConflicts`, `resolveConflict` | Sync-Modulgrenze vor React für accountgebundene reloadfeste Mutationen, revisionssicheren Delta-Autosave, idempotente Review-Events, Acknowledgements und vorbereitete Konfliktauflösung |
...
- `src/cloudRepository.test.js`: Tabellenmapping, Revision-/Geräte-Roundtrips, Medienreferenzen, Soft-Delete-Tombstones, Delta-Updates, stale Writes, append-only Review-Events, Acknowledgements und Voll-Replace.
- `src/syncEngine.test.js`, `src/syncOutbox.test.js`: Delete-freier Autosave-Flush, reloadfeste accountgebundene Queue, partielle Bestätigungen, Retry-Erhalt, serialisierte Flushes, append-only Merge und Geräte-Kontext.
