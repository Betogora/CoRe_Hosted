...
- Datei- und Dokumentverarbeitung ist bewusst begrenzt: PDF-/Text-Auslesung, visueller PDF-Viewer und browserseitige APKG-Medienpersistenz sind vorhanden; robuste DOCX-Extraktion, OCR, Bildregionen, erweiterte PDF-Werkzeuge, serverseitige APKG-Jobs, Medienexport/-sharing und globales Orphan-GC fehlen noch.
...
| `src/creationWorkflow.ts` | `createCreationWorkflow({ mediaStore, persistImportedDecks })` | APKG-Preview cached Medien lokal; Commit normalisiert Reimport-IDs, wartet auf die Cloud-Bestätigung der Stapel und startet danach die persistente Medien-Synchronisierung. Paste-Import, manuelle Dokumentanker, KI-Drafts und Draft-Annahme bleiben hinter derselben Creation-Seam. |
