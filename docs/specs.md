...
- `Neue Karten pro Tag`, `Wiederholungen pro Tag` und `Lernkarten vorziehen` werden ausschließlich in den Stapeleinstellungen bearbeitet und nicht im Sitzungs-Overlay angeboten. Die beiden Tagesgrenzen sind kompakte Ganzzahlfelder ohne Slider; `Maximales Intervall in Tagen` ist ebenfalls ein reines Ganzzahlfeld statt einer Auswahl. Die sichtbaren Grenzen sind 0 bis 500 neue Karten, 0 bis 2.000 Wiederholungen, 30 bis 36.500 Tage maximales Intervall und 0 bis 720 Minuten Vorziehfenster. Das Vorziehfenster bleibt ein Slider mit Zahlenfeld, ist standardmäßig 20 Minuten und wird durch `0` deaktiviert. Eine Änderung des Neulimits hebt einen alten Tages-Override auf. Im Abschnitt `Tagespensum und Reihenfolge` stehen diese vier Felder einschließlich Reihenfolge schmal in einer, mittel in zwei und breit in vier Spalten.
- Die Presets `Standard`, `Intensiv` und `Entspannt` setzen neue Karten, tägliche Wiederholungen und maximales Intervall auf `20 / 200 / 1.000`, `30 / 300 / 365` beziehungsweise `10 / 100 / 2.000`. Noch unveränderte eigene Einstellungen beginnen mit `20 / 200 / 1.000`. Benannte Presets werden auf diese kanonischen Werte aktualisiert; ausdrücklich gespeicherte eigene Werte bleiben erhalten.
