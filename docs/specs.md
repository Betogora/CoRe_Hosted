...
Ein Fragezeichen neben dem Theme-Schalter öffnet die direkt verlinkbare Hilfeseite `/hilfe` im normalen App-Shell-Inhaltsbereich. Ein kurzer Einstieg ordnet CoRes Ziel ein. Ein klar versetzter, leicht eingefärbter Kartenstapel fragt dort, welche Grundsätze CoRe für möglichst nachhaltiges Lernen nutzt, und beantwortet dies mit `Active Recall → Smarter Recall` sowie `Spaced Repetition → Content Repetition`. Zwei anschließende Scrollgeschichten erklären erst den wechselnden Abrufreiz und danach die FSRS-Lernkurve, ohne Workspace- oder Schedulerzustand zu verändern.
