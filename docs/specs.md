...
- Direkt unter `Aussetzen` zeigt `Nächste Fälligkeit` einen nativen, mit dem gespeicherten Lerntag vorbelegten Datumspicker. `Neu planen` wird erst für einen anderen, mindestens nächsten fachlichen Lerntag aktiv; ein heutiger oder überfälliger gespeicherter Termin bleibt bis dahin sichtbar. Die getrennte Aktion setzt `dueAt` DST-sicher auf den Lerntagesbeginn in Profilzeitzone, lässt eine Aussetzung bestehen und bestätigt den Erfolg unabhängig vom Inhaltsspeichern.
