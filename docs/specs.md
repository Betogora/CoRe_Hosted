...
E2E-Abnahme am 2026-07-11: Der lokale Docker-/Supabase-Lauf mit `npm run test:e2e:local` ist einschließlich Geräte-Registrierung beim Auth-Boot, duplikatfreiem Geräte-Reload, Fehlerfallback, PDF-Auswahl und den bestehenden Produktszenarien mit 19/19 Browser-Smokes grün. Der Runner startet nur die benötigten Supabase-Dienste, verarbeitet die aktuellen JSON-Statusdaten, schreibt lokale Schlüssel nicht in den normalen Start-Log und stoppt den Stack nach dem Lauf. Der Modul-Testlauf umfasst 252 bestandene Tests.

Cloud-Datenkorrektheit am 2026-07-11: `cloudRepository` mappt Revisionen, Soft-Deletes und Geräte-IDs bidirektional. Soft-gelöschte Rows bleiben als minimale `cloudTombstones` erhalten, erscheinen aber nicht in sichtbaren Collections. Der reguläre Autosave schreibt nur geänderte Rows, schützt Updates atomar über `user_id`, `id` und Basisrevision und führt bestätigte Serverrevisionen ohne Autosave-Loop zurück. `syncOutbox` speichert Mutationen accountgebunden und reloadfest mit Retry-Zähler; `reviewEventAppend` wird als idempotente Einzelmutation über `(user_id, id)` verarbeitet. Der Account-Boot registriert die stabile Browser-ID vor dem Cloud-Load in `sync_devices` und aktualisiert `last_seen_at`, User-Agent und das automatisch abgeleitete Label, ohne `created_at` zu ersetzen. Snapshot- und Review-Mutationen reichen getrennte IDs samt Geräte-ID an das Repository und werden nur nach dessen Acknowledgement entfernt. `replaceAccountCloudState()` bleibt der ausdrücklich destruktive Pfad für Legacy-Import und E2E-Reset. Konkrete Entity-Mutationen, Konflikterzeugung und Konflikt-UI fehlen weiterhin.

Ownership-/RLS-Abnahme am 2026-07-11: `npm run test:rls:local` startet ausschließlich Loopback-Supabase, wendet Migrationen an, führt `supabase/verify_schema_v1.sql` als fehlschlagendes Struktur-Gate aus und prüft anschließend die Data API mit zwei angemeldeten Testnutzern sowie einem sessionlosen `anon`-Client. Sieben grüne Smokes decken eigene Lese-/Schreibzugriffe, unsichtbare und unveränderbare fremde Rows, abgewiesene Ownership-Fälschung, verweigerte `anon`-Zugriffe, accountgebundene Deck-/Card-FKs, gleiche lokale IDs sowie Geräte-Heartbeat und gleiche Geräte-IDs in zwei Accounts ab. `npm run test:e2e:local` führt dasselbe Security-Gate vor Playwright aus; keine Service-Role-, Access- oder Refresh-Tokens gelangen in Vite, Playwright oder Fehlerartefakte. Das nächste aktive Ziel sind konkrete Cloud-Mutationen mit serverseitiger Revisionsprüfung.
...
Der lokale Compatibility-State trägt für revisionierte Entitäten `revision`, `deletedAt` und `updatedByDeviceId`, Review-Events zusätzlich `createdByDeviceId`. Beim Laden werden Soft-Deletes nicht in sichtbare Collections aufgenommen, sondern als minimale `cloudTombstones` erhalten. Reguläre Saves vergleichen den gemappten Inhalt mit dem Serverstand, überspringen unveränderte Rows und aktualisieren geänderte Rows nur mit passender Basisrevision; der bestätigte Server-State aktualisiert anschließend ausschließlich Sync-Metadaten und überschreibt keine zwischenzeitlich neueren lokalen Inhalte.
...
**Stand 2026-07-11:** Diese Punkte sind als Web-MVP weitgehend erfuellt und durch Modul-/Browser-Pruefungen abgedeckt. Neu abgesichert sind die Learning-Item-Creation-Pipeline, Legacy-Card-Normalisierung, Variantenanker, APKG-Zstd-Collection-Erkennung, Media-Manifeste, lokaler Browser-Medienspeicher, Reimport-Merge mit Erhalt lokaler Content-Edits, Pflichtlogin, accountgebundener Cache, lokaler Erstimportdialog, revisionsbedingter Sync-Engine-Autosave, die persistente Outbox mit idempotenten Review-Events und die accountgebundene Geräte-Registrierung. Ebenfalls umgesetzt sind Supabase-CLI-Link, Remote-Migrationen, RLS-/Policy-/Grant-Verify, Vercel-Projekt, Vercel-Env-Grenzen, Deployment und echter Supabase-Auth-/Profilpfad. Nicht Teil dieser DoD-Erfuellung sind konkrete Entity-Mutationen und Konfliktloesung, externe KI-Provider, serverseitige Jobs, produktive Community-Rechte, produktive/serverseitige APKG-Medienpersistenz, Admin-Werkzeuge und Observability. Die relevanten Produktivleitplanken fuer Vercel, Supabase/Postgres/RLS, Storage, Secrets und KI-Proxying sind in dieser Spec dokumentiert; die Umsetzungsluecke wird in `docs/todo.md` als Ausbaupfad gefuehrt.
...
1. Persistenten Sync ausbauen: konkrete Deck-/Card-/Delete-Mutationen mit serverseitiger Revisionsprüfung, Konfliktzeilen und Konfliktauflösung auf der fertigen Outbox und Geräte-Registrierung aufbauen.
...
| `src/cloudRepository.js` | `registerAccountSyncDevice`, `replaceAccountCloudState`, `upsertAccountCloudState`, `appendReviewEvent`, `loadAccountCloudState`, `mergeCloudSyncMetadata`, `listAccountSyncConflicts`, `resolveAccountSyncConflict`, Mapping-Helfer | Accountgefiltertes Laden, Geräte-Upsert, unsichtbare Soft-Delete-Tombstones, revisionsbedingte Delta-Updates, getrennte Snapshot-/Review-Acknowledgements und bewusster Voll-Replace fuer Legacy-Import/Test-Reset |
| `src/syncEngine.js`, `src/syncOutbox.js` | `createAccountSyncEngine`, `createSyncEngine`, `createSyncOutbox`, `loadSnapshot`, `enqueueMutation`, `flush`, `listConflicts`, `resolveConflict` | Sync-Modulgrenze vor React für Geräte-Registrierung vor dem Cloud-Load, accountgebundene reloadfeste Mutationen, revisionssicheren Delta-Autosave, idempotente Review-Events, typsichere Acknowledgements und vorbereitete Konfliktauflösung |
...
- `src/cloudRepository.test.js`: Tabellenmapping, Geräte-Registrierung/Heartbeat, Account-Isolation, Revision-/Geräte-Roundtrips, Medienreferenzen, Soft-Delete-Tombstones, Delta-Updates, stale Writes, append-only Review-Events, getrennte Acknowledgements und Voll-Replace.
- `src/syncEngine.test.js`, `src/syncOutbox.test.js`: Registrierung vor Snapshot-Load, blockierter Startfehler, gemischte Snapshot-/Review-Batches, fehlende Acknowledgements, reloadfeste accountgebundene Queue, Retry-Erhalt, serialisierte Flushes und Geräte-Kontext.
