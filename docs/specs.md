...
1. Persistenten Sync ausbauen: auf den revisionsgeprüften Deck-/Card-/Delete-Mutationen und erzeugten Konfliktzeilen jetzt explizite Medien-/Feld-Merge-Regeln, Konfliktauflösung, Wiederverbindungs-Backoff und Zwei-Geräte-Szenarien aufbauen.
...
| `src/cloudRepository.js` | `registerAccountSyncDevice`, `applyDeckMutation`, `applyCardMutation`, `appendReviewEvent`, `softDeleteEntity`, `markConflict`, `replaceAccountCloudState`, `upsertAccountCloudState`, `loadAccountCloudState`, `mergeCloudSyncMetadata`, `listAccountSyncConflicts`, `resolveAccountSyncConflict`, Mapping-Helfer | Accountgefiltertes Laden, Geräte-Upsert, serverseitiges Compare-and-set, idempotente Konfliktzeilen, unsichtbare Soft-Delete-Tombstones, getrennte Snapshot-/Review-Acknowledgements und bewusster Voll-Replace fuer Legacy-Import/Test-Reset |
...
- `src/cloudRepository.test.js`: Tabellenmapping, Geräte-Registrierung/Heartbeat, Account-Isolation, konkrete Deck-/Card-Mutationen, CAS-Revisionen, idempotente Retries und Konfliktzeilen, Soft-Delete-Tombstones, readback-bestätigte Review-Events, getrennte Acknowledgements und Voll-Replace.
- `src/syncEngine.test.js`, `src/syncOutbox.test.js`: Registrierung vor Snapshot-Load, blockierter Startfehler, Review-vor-Snapshot-Reihenfolge, partielle Acknowledgements bei Konflikten, fehlende Bestätigungen, reloadfeste accountgebundene Queue, Retry-Erhalt, serialisierte Flushes und Geräte-Kontext.
