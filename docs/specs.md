...
- Unter 1280 px ersetzt eine schwebende, Safe-Area-fähige Bottom Bar die Sidebar. Sie bleibt unabhängig von Seitenlänge und vertikaler Browser-Scrollbar mit stabiler Breite am unteren Viewportrand sichtbar. Ihre fünf direkten Ziele sind `Heute`, `Lernen`, `Erstellen`, `Statistik` und `Mehr`; `Mehr` öffnet ohne Zwischenmenü die Kartenverwaltung. Ein kompakter Kopf zeigt `CoRe`, bei aktivem Timer dessen kleinen Fortschrittsbalken sowie rechts die gemeinsame Utility-Gruppe aus umrundetem Einstellungsrad und randlosem Theme-Button. Eine aktive Simulation bleibt in der eigenen Statuszeile sichtbar. Ab 1280 px bleibt die Sidebar mit denselben fünf Hauptzielen, dem Pomodoro-Fortschritt oberhalb der Trennlinie und derselben rechtsbündigen Utility-Gruppe erhalten; eine Profilvorschau erscheint dort nicht.
