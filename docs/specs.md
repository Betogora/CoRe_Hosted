...
Vor der Antwort zeigt der Review ausschließlich den Lerninhalt und die Aktion zum Aufdecken. Der Lerninhalt liegt direkt auf der Seitenfläche ohne großen Kartencontainer und ohne gerahmte Teilflächen für Frage oder Antwort. Nach dem Aufdecken bleiben Frage und Antwort sichtbar, getrennt durch eine markante 2-px-Linie; die Antwortprojektion enthält ausschließlich die Antwort und wiederholt die bereits sichtbare Frage nicht. Vier Bewertungen aktualisieren den Lernzustand.
...
- Die erste Methodenüberschrift lautet ausschließlich `Active Recall`. Ihre Geschichte besitzt drei aufeinanderfolgende Textschritte und eine rahmenlose Kartenvisualisierung: Zuerst zeigt sie eine klar lesbare, leicht eingefärbte Karte mit orange hervorgehobenen Schlüsselstellen, danach dieselbe vertraute Form mit verpixeltem neutralem Fragetext und schließlich zwei deutlich versetzte Variantenkarten mit derselben Antwort und gelben Sternen in den Ecken. Beim Verpixeln bleiben Textfluss, Zeilenumbrüche, Trennlinie und Antwort exakt an ihrer vorherigen Position. Die nicht aktiven Textschritte bleiben gut lesbar. Ab Desktopbreite bleibt die Kartenvisualisierung beim Scrollen innerhalb dieses Abschnitts stehen und wechselt synchron zum aktiven Textschritt; darunter bleibt die Abfolge linear. Der Kartenstapel im Einstieg endet mit allen drei Karten auf derselben unteren Kante; die beiden hinteren Karten sind unterschiedlich hoch und seitlich versetzt, sodass ihre Staffelung ausschließlich nach oben sichtbar wird.
- Die beiden Methodeneinträge im Einstieg sind als Abschnittsnavigation bedienbar. Beide oberen Linien bleiben im Ruhezustand dünn; nur der mit Maus oder Tastatur fokussierte Eintrag erhält kräftigere Schrift und eine stärkere obere Linie. `Active Recall` scrollt in einer deutlich wahrnehmbaren Bewegung zu seiner Methodenüberschrift; bei reduzierter Bewegung erfolgt der Wechsel unmittelbar. `Spaced Repetition` springt immer direkt zum zweiten Methodenabschnitt, damit die drei Active-Recall-Unterslides beim Überspringen nicht nacheinander abgespielt werden.
...
- Sobald ein globales oder stapelbezogenes Einstellungsfeld vom letzten gespeicherten Seitenzustand abweicht, erscheint seitenweit genau eine zweizeilige, nichtmodale Leiste `Ungespeicherte Änderungen` im unteren Viewportbereich. `Speichern` übernimmt sämtliche Änderungen der Seite gemeinsam; `Verwerfen` stellt sämtliche Felder auf den neuesten gespeicherten Stand zurück. Während des Speicherns sind beide Aktionen gesperrt. Erfolg schließt die Leiste und meldet genau einmal `Globale Einstellungen wurden gespeichert.` beziehungsweise `Stapeleinstellungen wurden gespeichert.`; Validierungs- und Speicherfehler lassen sie offen und erscheinen beim betroffenen Bereich.
- App-Navigation, Querlinks, Zurück, Stapelwechsel und Browser-Zurück/Vorwärts halten bei ungespeicherten Einstellungen ihr Ziel zurück. Erfolgreiches Speichern oder Verwerfen setzt die vorgemerkte Navigation automatisch fort. Reine Hashnavigation innerhalb der Einstellungsseite bleibt frei; Reload, Tab-Schließen und externe Navigation verwenden die native Browserwarnung. Lernprofil-CRUD, `Auf diesen Stapel anwenden`, manueller Sync, Export, Import und die Lerneinstellungen einer Kartensitzung behalten ihre unmittelbaren eigenen Aktionen und erzeugen keine Einstellungsleiste.
