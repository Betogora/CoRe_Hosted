...
`Lernen` ist der primäre schnelle Einstieg in eine Sitzung. `Karten` ist die fünfte Hauptseite und die direktlinkfähige Verwaltungsoberfläche für alle Karten, Stapelstruktur, Inhalt, Herkunft und erweiterte Optionen. Die bestehende Route bleibt `/kartenstapel`; in der mobilen Bottom Bar verwendet der direkte Einstieg ebenfalls die Bezeichnung `Karten` und das Stapel-Icon.
