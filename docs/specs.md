...
**Stand 2026-07-10:** Diese Punkte sind als Web-MVP weitgehend erfuellt und durch Modul-/Browser-Pruefungen abgedeckt. Neu abgesichert sind die Learning-Item-Creation-Pipeline, Legacy-Card-Normalisierung, Variantenanker, APKG-Zstd-Collection-Erkennung, Media-Manifeste, lokaler Browser-Medienspeicher, Reimport-Merge mit Erhalt lokaler Content-Edits, Pflichtlogin, accountgebundener Cache, lokaler Erstimportdialog und revisionsbedingter Sync-Engine-Autosave mit Tombstones, append-only Review-Events und Server-Acknowledgements. Ebenfalls umgesetzt sind Supabase-CLI-Link, Remote-Migrationen, RLS-/Policy-/Grant-Verify, Vercel-Projekt, Vercel-Env-Grenzen, Deployment und echter Supabase-Auth-/Profilpfad. Nicht Teil dieser DoD-Erfuellung sind persistente Offline-Outbox und Konfliktloesung, externe KI-Provider, serverseitige Jobs, produktive Community-Rechte, produktive/serverseitige APKG-Medienpersistenz, Admin-Werkzeuge und Observability. Die relevanten Produktivleitplanken fuer Vercel, Supabase/Postgres/RLS, Storage, Secrets und KI-Proxying sind in dieser Spec dokumentiert; die Umsetzungsluecke wird in `docs/todo.md` als Ausbaupfad gefuehrt.
...
| `src/syncEngine.js` | `createAccountSyncEngine`, `createSyncEngine`, `enqueueMutation`, `flush`, `listConflicts`, `resolveConflict`, `mergeAppendOnlyRows`, `detectRevisionConflict` | Sync-Modulgrenze vor React fuer Snapshot-Load, in-memory Mutation Queue, revisionssicheren Delta-Autosave, Acknowledgements und vorbereitete Konfliktauflösung |
...
- `src/syncEngine.test.js`: Delete-freier Autosave-Flush, append-only Merge, Revision-Konflikterkennung sowie Geräte-Kontext und bestätigter Server-State.
