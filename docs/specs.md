...
Produktreife-Abgleich am 2026-07-15: Der breite lokale Web-MVP ist in `Core`, `Labs` und `Disabled` getrennt. `src/productSurfaces.ts` ist die typisierte Quelle fuer Reife, Hauptnavigationssichtbarkeit, bekannte Grenzen und explizite Environment-Freigaben. P0.1, P0.2 und P0.4 sind umgesetzt; die verbleibenden Produkt- und UX-Gates stehen in `docs/todo.md`. Klickbare lokale oder technisch vorbereitete Flows gelten nicht allein deshalb als freigegebener Produktkern.
...
P1.3-Testportfolio am 2026-07-15: TypeScript-Tests sind ausführbar in `unit`, `contract` und `integration` geordnet; genau fünf mit dem geschützten Vertrag benannte Golden-E2Es bilden das verpflichtende PR-Produktgate. Das PR-Gate führt Typecheck, Unit/Contract, Build, Kern-RLS und Golden-E2E aus. Vollständige Browser-, Zwei-Geräte-, TUS-über-6-MB-, Medien-, Restore- und Groß-APKG-Pfade bleiben als `heavy-release` auf `main`, Nightly, veröffentlichten Releases und manuellen Läufen verpflichtend. Die Inventur mit Produktvertrag, Laufzeit und Infrastruktur steht in `docs/test-portfolio.md`; Testanzahlen sind kein Abnahmekriterium.

P0.4-Erstkontakt am 2026-07-15: Produktive und standardmaessige Repository-Zustaende starten ohne Demo-Stapel, erfundene Profildaten oder fremde Lernhistorie. Die Welt-Hauptstadt-Fixture bleibt ueber `seedDefaultDecks: true` fuer lokale Entwicklung und E2E sowie ohne fremde Review-Events ueber eine ausdrueckliche Demo-Aktion verfuegbar. Das leere Dashboard bietet Anki-Import, erste manuelle Karte und bewusste Demo, erklaert das Kernversprechen in drei kurzen Punkten und zeigt noch keine Statistik. Nach manueller Erstellung oder APKG-Commit bleibt eine URL-gestuetzte Bestaetigung mit `Jetzt lernen` und `Karten prüfen` ueber Reload und Browser-History erhalten; Hochschule und Fachgebiet bleiben optionale Profildaten ausserhalb des Lernstarts.

P0.5-Erstellen am 2026-07-15: `Manuell` und `Import` sind kompakte Core-Einstiege; der `Lokale Entwurfsassistent` bleibt als Labs-Fläche klar lokal und deterministisch und ruft kein externes Modell auf. Quellpicker bieten ausschließlich lesbare PDF-, Text-, Markdown-, CSV- und TSV-Dateien an; DOCX bleibt bis zu einer echten Extraktion nicht auswählbar. Der APKG-Hauptbericht zeigt Datei, Stapel, Karten, vorhandene und fehlende Medien sowie verständliche Warnungen. Notetype-IDs, Template-Ordinals, Hashes, Reimportwerte und Importidentitäten bleiben unter `Technische Details` verfügbar. Der Status läuft über Analysieren, Vorschau, Übernehmen und Mediensynchronisierung zu fertig, teilweise fertig oder fehlgeschlagen; Fehlerzustände bieten genau eine empfohlene Primäraktion. Erfolgreiche manuelle und APKG-Flows enden weiter mit `Jetzt lernen` als primärer Folgeaktion.
...
| Decks, Hierarchie, CoRe-Modus | `menuModel`, `DecksScreen`, `LearnScreen`, `DeckSettingsScreen`, `deckSettings`, `libraryModel`, `coreWorkspace.renameDeck`, `coreWorkspace.moveDeck`, `coreModel.createCoreDeck`, `deckSettings.coreMode`, `deckSettings.appearance`, echte Parent-/Child-Stapel aus APKG-Hierarchien, manuelle Unterstapel-Anlage hinter einem Button, Icon-/Farb-Customizing in den Stapeloptionen, direkte Umbenennung, isolierte Lernoptionen je Stapel, aufklappbare und gestaffelt hinterlegte Lernuebersicht mit eindeutigem Lernstart sowie explizites Verschieben in der Verwaltung, Learning-Item-Normalisierung |
...
- Dashboard mit neutralem Heute-Kopf und einem statistikfreien Leerzustand fuer Anki-Import, erste Karte oder ausdrueckliche Demo; ab eigenem Inhalt erscheinen Fällig-/Originalkarten-Kacheln, aktive Hauptstapel mit aggregierten Unterstapel-Zahlen und die GitHub-/Anki-artige Jahres-Heatmap aus lokalen Review-Events.
...
- Deck-Verwaltung mit Suche/Filter, sichtbarer Parent-/Child-Hierarchie, explizitem und bestaetigtem `Verschieben` samt Zielauswahl, direktem Umbenennen, Stapelbaum-Loeschen, aggregierten Elternstapel-Zahlen, CoRe-Modus pro Stapel, nachgelagerter Icon-/Farbwahl sowie Kartenbearbeitung mit rueckverfolgbaren Restore-Versionen. Technische Varianten-, Prompt- und Schedulerwerkzeuge liegen eingeklappt unter `Labs / Erweitert`. Die Lernuebersicht zeigt dieselbe Hierarchie als aufklappbare, gestaffelt hinterlegte Stapelliste mit spaltenbündigen Neu-, Faellig- und Gesamtzahlen, einem klaren Lernstart und sekundaeren Stapeloptionen; sie enthaelt keine Struktur-Geste. Die schnelle Stapelanlage fragt nur Name und optionalen Elternstapel ab.
- Produktive und standardmaessige neue Zustaende starten leer. Die reproduzierbare `Welt-Hauptstädte`-Fixture mit 245 Karten und sieben Kontinent-Unterstapeln bleibt nur opt-in fuer lokale Entwicklung, E2E und den ausdruecklichen Demo-Klick; die Demo-Kopie uebernimmt keine fremden Review-Events.
...
- Erstellen-Screen mit history-stabiler Vorauswahl für manuelle Erstellung oder Import, kompakten Core-/Labs-Einstiegen und einer reloadfesten Abschlussbestätigung mit `Jetzt lernen` und `Karten prüfen`; manuelle Kartenerstellung mit optionaler PDF-/Textquelle, Anki-artig untereinander angeordneter Vorder- und Rückseite, Pinnfunktion, Rich-Text-Editor und bestehendem/neuem Stapelziel.
...
Die technische Existenz eines Screens, Modells oder lokalen Flows aendert seinen Reifestatus nicht. Die zentrale typisierte Produktflaechen-Registry `src/productSurfaces.ts` steuert Sichtbarkeit, Kennzeichnung und explizite Freigaben. Nur diese Registry liest die zugehoerigen Vite-Environment-Schalter; Screens erhalten daraus abgeleitete Freigaben.
...
In der Stapelverwaltung MUESSEN Stapel direkt umbenannt, manuell als Haupt- oder Unterstapel angelegt, ueber eine sichtbare Aktion `Verschieben` mit Zielauswahl und eigener Bestaetigung neu eingeordnet und als ganzer Stapelbaum geloescht werden koennen. Zielauswahl und Bestaetigung MUESSEN per Tastatur und Screenreader bedienbar sein; der eigene Stapel und seine Unterstapel duerfen nicht als Ziel angeboten werden.

Die Lernuebersicht DARF keine Drag-, Drop- oder andere Struktur-Geste auf einer Lernzeile anbieten. Ein Klick auf den nicht interaktiven Zeilenbereich oder den sichtbaren Lernbutton startet ausschliesslich die Lernsitzung; ein sekundaerer Button oeffnet die Stapeloptionen.
...
Die direkte Stapel-Anlage in der Lernuebersicht SOLL initial hinter einem Button verborgen sein und nur Name sowie einen optionalen Elternstapel abfragen. Stapel-Icon und Iconfarbe werden nachgelagert in den Einstellungen des gewaehlten Stapels bearbeitet.
...
**Lokaler Stand 2026-07-15:** Die Kartenverwaltung bietet fuer versionierte Inhaltsaenderungen eine Versionsauswahl mit sichtbarer Aktuell-/Nach-Restore-Zusammenfassung und einer separaten Bestaetigung. Der Restore laeuft ueber `coreModel.restoreCardVersion` und `coreWorkspace.restoreDeckCardVersion`; er ersetzt keinen Verlaufseintrag, sondern haengt einen neuen Eintrag mit `changeType: "version_restored"` und Vorher-/Nachher-Snapshot an.
...
| `src/coreRepository.ts` | `createCoreRepository()` | Persistenter lokaler App-State, Migration alter Decks, neutraler leerer Standardzustand, ausdruecklicher Fixture-Seed ueber `seedDefaultDecks: true`, Profile, Communities, Jobs, Dokumente, Chat und Lernplaene |
...
| `src/authenticatedWorkspaceBoot.ts` | `bootAuthenticatedWorkspace`, `startAuthenticatedWorkspaceSessionLifecycle` | Konkreter accountgebundener Session-/Recovery-Lifecycle und Boot von lokalem Repository, `coreWorkspace`, SyncEngine, Cloud-Snapshot, Konfliktanzahl und optionaler Legacy-Übernahme; Unmount invalidiert laufende Boots und entfernt das Auth-Abonnement |
...
| `src/appSyncLifecycle.ts` | `startAppSyncLifecycle`, `startAppAutosaveLifecycle` | App-seitige React-Verkabelung von SyncEngine und verzögertem Autosave mit Cleanup bei Accountwechsel, Abmeldung und Unmount; fachliche Retry-, Konflikt- und Flush-Regeln bleiben in `syncEngine` |
| `src/coreWorkspace.ts` | `createCoreWorkspace`, `createDemoAnatomyDeck`, `createWorldCapitalsDemo`, `createDeck`, `renameDeck`, `moveDeck`, `deleteDeckTree`, `setDeckCoreMode`, `saveDeckCardContent`, `deleteDeckCard`, `addManualCardToDeck`, `dryRunApkgImport`, `commitApkgImport`, `importApkgDeck`, `applyVariantGenerationResponse` | Lokale App-Kommandos fuer ausdrueckliche Demo-Daten, Stapelpflege, Kartenpflege und KI-Variantenannahme; die bestehenden asynchronen APKG-Methoden laden APKG-/SQLite-/Zstd-Code erst bei Nutzung |
...
| `src/documentModel.ts` | `READABLE_SOURCE_DOCUMENT_ACCEPT`, `READABLE_SOURCE_DOCUMENT_LABEL`, `isTextReadableFile`, `isPdfFile`, `formatPdfTextContentItems`, `createDocumentFromFile`, `createAnchorFromSelection`, `splitDocumentIntoPassages` | Dokumente, zentraler Picker-Vertrag für tatsächlich lesbare Dateitypen, PDF-Textformatierung und Auswahl-zu-Quelle; PDF.js wird über die geteilte Runtime bedarfsgeladen |
...
| `src/appMediaLifecycle.ts` | `startAppMediaRetryLifecycle` | App-seitige Verkabelung des accountgebundenen Media-Retry-Lifecycles mit Cleanup und Projektion bestätigter Cloud-Referenzen zurück in bestehende Decks; Queue- und Uploadregeln bleiben im `mediaStore`. |
...
| `src/useAppNavigation.ts`, `src/appScreenProps.ts` | `useAppNavigation`, `projectAppRoute`, `subscribeToBrowserNavigation`, konkrete Lazy-Screen-Props | Projektspezifische Browser-History-/Route-Lifecycle-Verkabelung und ausschließlich typisierte Shell-zu-Screen-Komposition; keine neue öffentliche Daten-, Provider- oder Routing-Seam |
| `src/productSurfaces.ts` | `ProductSurface`, `ProductSurfaceRegistry`, `createProductSurfaceRegistry`, `productSurfaces` | Zentrale Reife-, Sichtbarkeits- und Environment-Freigabe fuer Core-, Labs- und Disabled-Flaechen |
...
`src/App.tsx` ist primär die App-Shell für Workspace-State, Workspace-Kommandowiring, Route-Auswahl und Screen-Komposition. Die zuvor direkt enthaltenen Verantwortungsblöcke waren Account-/Cloud-Boot, Browser-History/Navigation, Sync/Autosave und Media-Retry; sie liegen nun in `authenticatedWorkspaceBoot.ts`, `useAppNavigation.ts`, `appSyncLifecycle.ts` und `appMediaLifecycle.ts`. Diese Module verkabeln ausschließlich die bestehenden fachlichen Grenzen: Domänenkommandos bleiben in `coreWorkspace`, Syncregeln in `syncEngine`, Medienregeln in `mediaStore` und der Routingvertrag in `appNavigation`. Nur Login-Gate und Shell sind eager; authentifizierte Screens und `StudyMode` werden mit konkreten Props-Verträgen per `React.lazy` hinter einem deutschen `Suspense`-Status geladen. `src/main.tsx` umschliesst die Shell mit `AppErrorBoundary`. Die Shell nutzt die verfuegbare Browserbreite statt einer festen Desktop-Maximalbreite; `StudyMode` folgt demselben Full-Width-Verhalten. Produktnahe UI liegt in `src/screens/`; geteilte Praesentationsbausteine, Medien-HTML, `PdfDocumentViewer` und `ReleaseInfo` liegen in `src/ui/`. Die Screen-Map fuer KI-Programmierung wird in `src/screens/README.md` gepflegt. Screens verwenden die Module oben als Test- und Implementierungsoberflaeche und sollen keine APKG-, PDF.js-, Medien-, Scheduler-, Varianten- oder Persistenzdetails ausbreiten.
...
- `src/screens/DashboardScreen.tsx`: neutraler Heute-Kopf, statistikfreier Leerzustand mit Import/Karte/Demo und drei kurzen CoRe-Punkten; ab eigenem Inhalt Fällig-/Originalkarten-Kacheln, Heatmap, aktive Hauptstapel und Stapel-Icons/-Farben.
- `src/screens/DecksScreen.tsx`: Deck-Hierarchie, Suche/Filter, explizites bestaetigtes Verschieben mit Zielauswahl, direkte Stapel-Umbenennung, Stapelbaum-Loeschen, CoRe-Modus, Karten-CRUD, Versionsvergleich/-Restore sowie eingeklappte technische Varianten-, Prompt- und Schedulerwerkzeuge unter `Labs / Erweitert`.
- `src/screens/CreationScreen.tsx`: APKG bis zur freigegebenen Groessengrenze, Text/CSV/Excel-Paste, manuell, PDF-/Text-Dokumenterstellung mit Quellenanker ueber den tiefen PDF.js-Viewer sowie gemeinsamer Abschluss mit Lernen-/Pruefen-Aktionen; lokale KI-Drafts nur in Labs.
- `src/screens/LearnScreen.tsx`: Aufklappbare Lernuebersicht fuer Stapel und Unterstapel mit hellen Gruppenhintergruenden, spaltenbündigen Neu-/Faellig-/Gesamtzahlen, sichtbarem Lernbutton, klickbarem nicht-interaktivem Zeilenbereich, sekundaeren Stapeloptionen und schneller Anlage nur aus Name plus optionalem Elternstapel; keine Drag-/Drop-Strukturaenderung.
- `src/screens/DeckSettingsScreen.tsx`: Isolierte Einstellungsseite fuer genau einen Stapel mit Stapel-Icon/-Farbe, gemeinsamem `LearningSettingsPanel`, Tagespensum, Queue-Reihenfolge, CoRe-Modus, Lern-/Wiederlernintervallen und Zielerinnerung.
...
Die verbindliche Testinventur, Kategorien, Laufzeitkorridore und Ausführungsfrequenzen stehen in `docs/test-portfolio.md`. `scripts/runModuleTests.ts` ordnet die TypeScript-Dateisuites ausführbar den Kategorien `unit`, `contract` und `integration` zu. Playwright markiert genau fünf Nutzerziele mit `@golden-e2e`; deren Namen beginnen mit `[Vertrag: …]`. `scripts/runLocalE2E.ts` trennt `pr`, `golden`, `core-rls`, vollständiges `rls` und `release`, ohne RLS-Strukturprüfung oder lokale URL-/Secret-Grenzen zu umgehen. Das PR-Gate enthält Typecheck, Unit/Contract, Build, Kern-RLS und Golden-E2E. Das vollständige Release-Gate ergänzt Integration, alle RLS-/Zwei-Geräte- und Browserflows, TUS über 6 MB, Medien-/Restore-Pfade sowie den reproduzierbaren Groß-APKG-Benchmark.
...
- `tests/e2e/core-stabilization.spec.ts`: authentifizierte Browser-Smokes fuer Navigation, Review-Rating, manuell vorbereitete Varianten-Session unter Labs, bestaetigten und auditierbaren Version-Restore, getrennte KI-Draft- und Assistenten-Flows, lokalen JSON-Export/-Import mit Fehlerstatus sowie PDF-Lazy-Loading, Textauswahl, Kartenfeld und gespeicherten Quellenanker.
- `tests/e2e/world-capitals-hierarchy.spec.ts`: authentifizierter Browser-Smoke fuer den deterministisch zurueckgesetzten Cloud-Seed, sichtbare Unterstapel, gestaffelte Lernlisten-Gruppenhintergruende, spaltenbündige Lernlisten-Zahlen, expliziten Lernstart, dragfreie Lernzeilen, isolierte Einzelstapel-Einstellungen und bestaetigtes Verschieben/Outdent in der Verwaltung.
...
Der Screen `Erstellen` zeigt zuerst drei kompakte Auswahlkarten: `Karten manuell erstellen` und `Import` sind als Core gekennzeichnet, der `Lokale Entwurfsassistent` als Labs. Die drei Karten bleiben bei 1440 × 900 und 1280 × 720 vollständig sichtbar und führen mit kleinem Icon, Reifehinweis, Titel und kurzer Beschreibung in den jeweiligen Arbeitsbereich. Nach Auswahl führt ein Auswahl-Knopf zurück. Manuelle Kartenerstellung und PDF-/Text-Erstellung sind zusammengeführt: Im manuellen Arbeitsbereich kann per `PDF/Text anfügen` eine tatsächlich lesbare PDF-, Text-, Markdown-, CSV- oder TSV-Quelle angehängt werden; DOCX ist bis zu einer implementierten Extraktion nicht auswählbar. Im Importbereich werden APKG, Text, CSV und Excel-/Tabellen-Paste als Unterauswahl angeboten. Die manuelle Kartenerstellung unterstützt bestehende Stapel oder einen neuen Stapel als Ziel und erzeugt Basic, Umgekehrt, Lückentext und Multiple Choice über die gemeinsame Learning-Item-Creation-Pipeline. Vorder- und Rückseite stehen Anki-artig untereinander, wachsen nur über ihren Inhalt und können jeweils per Pinnadel nach dem Speichern für die nächste Karte behalten werden. Beide Seiten nutzen einen lokalen Rich-Text-Editor für Fett, Kursiv, Unterstreichung, Stichpunkte, nummerierte Listen, CoRe-eigene kompakte Farbpopover mit je drei lokal gespeicherten und per Farbspektrum, Schnellfarben oder Hex-Eingabe anpassbaren Stift-/Markerfarben sowie Formatierung entfernen; gespeichert wird weiter sanitisiertes Karten-HTML. Multiple Choice markiert im Review gewählte und richtige Antworten sichtbar; Free Text wird nicht mehr angeboten und alte Free-Text-Daten laufen defensiv als Basic. Image occlusion bleibt bewusst für einen späteren separaten Ausbau.
...
Der Labs-Flow `Lokaler Entwurfsassistent` ist review-first. Er kennt Parameter für Sprache, Kartenanzahl, Detailgrad, Kartentypen, Fokus und Fach, aber kein Kostenprofil. Es wird kein externes Modell aufgerufen. `src/aiOrchestrator.ts` erzeugt lokal deterministische, strukturierte Entwürfe aus Quellentext, validiert das erwartete JSON-Schema und speichert Quellenanker. `createAiDraftDeck` führt die Entwürfe durch dieselbe Learning-Item-Creation-Pipeline wie manuelle und importierte Inhalte. Generierte Karten behalten `draftStatus: "draft"`, bis der Nutzer sie akzeptiert.
...
- Versionierter Importbericht in `CreationScreen`: Der Hauptbericht zeigt Datei und Größe, erkannte Stapel, Kartenanzahl, vorhandene/fehlende Medien, verständliche Warnungen und `Import übernehmen`. Notetype-IDs, Template-Ordinals, Hashes, Reimportwerte und ausführliche Importidentitäten liegen eingeklappt unter `Technische Details`; React rendert weiterhin nur diese Projektion und den `mediaStore`-Status.
