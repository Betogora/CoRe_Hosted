...
- Enthält eine APKG-Hierarchie Quellstapel ab Ebene 9, nennt die Vorschau einmalig deren Anzahl und erklärt, dass sie auf Ebene 8 abgeflacht werden, während ihre ursprünglichen Anki-Pfade erhalten bleiben.
...
- Im Kopf der manuellen Erstellungsfläche öffnet `Vorschau` den gemeinsamen modalen Karten-Vorschau-Dialog aus dem aktuellen ungespeicherten Entwurf. Text, Kartentyp, Zusatzfelder und frei positionierte Inline-Bilder stammen dabei aus derselben Projektion wie beim Speichern. Eine zusätzliche aufgeklappte `Live-Vorschau` wird in der Kartenerstellung nicht angezeigt. Die modale Vorschau verzichtet auf pauschale Sicherheits- oder Originaltreue-Werbung. `PDF/Text anfügen` steht in der oberen Zurückzeile direkt neben `Erstellen`.
- Die manuelle Erstellung beginnt ohne Kartentypauswahl mit Vorder- und Rückseite. `Fragentyp` und `Lernrichtung` stehen ab 768 px gemeinsam in einer Zeile und darunter in zwei sauber umbrechenden Gruppen. Bilder werden über die Rich-Text-Toolbar, Einfügen oder Drag-and-drop an der aktuellen Cursorposition in Vorderseite, Rückseite und Zusatzfelder eingefügt; separate Bild-Ablagefelder existieren nicht. `Lücke` markiert die gespeicherte Textauswahl als Cloze-Gruppe, Rückrichtung, Single Choice und Multiple Choice werden als direkte Steuerungen angeboten. Bei beiden Auswahltypen stehen die Antwortoptionen unmittelbar unter der Frage. Zusätzliche benannte Felder erhalten eine Platzierung auf Vorderseite, Rückseite, beiden Seiten oder nur in Metadaten; `Feld hinzufügen` steht ohne ausklappbare Gruppenfläche direkt darunter.
...
`Lernen` ist der primäre schnelle Einstieg in eine Sitzung und bündelt die beiden Bereiche `Stapelübersicht` und `Kartenverwaltung`. Die Auswahl erfolgt über ein reguläres segmentiertes Control rechts neben der Überschrift `Lernen`; die Route leitet den aktiven Bereich ab. Daneben öffnet der beschriftete Button `Lerneinstellungen` die accountweiten Lern- und Planungseinstellungen unter dem kompatiblen Pfad `/karten-einstellungen`. Die direktlinkfähige Kartenverwaltung für alle Karten, Stapelstruktur, Inhalt, Herkunft und erweiterte Optionen behält die verborgene Route `/kartenstapel`.
...
- Suche berücksichtigt aktuellen Stapelpfad, bei abgeflachten Importen zusätzlich den ursprünglichen Anki-Pfad, Vorderseite, Rückseite und Tags und öffnet passende Sektionen nur für die Dauer der Suche. Die Kartenverwaltung besitzt keinen zusätzlichen Modusfilter.
...
- Die nichtmodale Speicherleiste der Stapeleinstellungen bezeichnet ihre stapelbezogene Aktion ausdrücklich. Besitzt der gewählte Stapel Unterstapel, ist `Stapel und Unterstapel speichern` die dunkel hinterlegte Hauptaktion vor der sekundären Aktion `Nur diesen Stapel speichern`. Die Hauptaktion übernimmt rekursiv nur die seit dem gespeicherten Stand geänderten Darstellungs-, Lern-, Scheduler- und CoRe-Werte; Unterstapelnamen sowie nicht geänderte individuelle Werte bleiben erhalten. Ohne Unterstapel erscheint nur `Stapeleinstellungen speichern`. Beide Wege speichern lokal persistent in einer gemeinsamen Mutation, bleiben auf der Einstellungsseite und bestätigen ihren jeweiligen Geltungsbereich. Das Kreuz rechts oben verwirft den offenen Entwurf bewusst und stellt den zuletzt gespeicherten Stand wieder her.
...
- Gleichnamige Unterstapel werden in relevanten Links und Auswahlen durch ihren vollständigen Hierarchiepfad unterschieden; abgeflachte Importe ergänzen zur eindeutigen Herkunft den ursprünglichen Anki-Pfad.
- Stapelbezogene Auswahlfelder zeigen im geschlossenen Zustand Stapel-Icon und vollständigen Hierarchiepfad. Für abgeflachte Importe lautet die Beschriftung `<aktueller Pfad> · Anki: <ursprünglicher Pfad>` und beide Pfade sind durchsuchbar. Das geöffnete Feld ist eine kompakte, höchstens 320 px hohe, innerhalb jeder Hierarchieebene alphabetisch sortierte Baumliste mit nativer Seiten-Scrollbar, individuellem Stapel-Icon, einzeiligem Namen und Einrückung je Hierarchieebene; ohne tatsächlichen Überlauf nutzen Zeilenflächen die vollständige Listenbreite. Ab fünf tatsächlich auswählbaren Ober- und Unterstapeln erscheint eine Suche über den vollständigen Pfad. Sonderziele wie Hauptstapel oder Hauptebene zählen nicht zur Suchschwelle. Single- und Statistik-Multi-Select markieren ausgewählte Zeilen mit einer neutralen Auswahlfläche und einem Haken rechts. In der Statistik stehen Suche, `Gesamte Sammlung` und Stapelbaum untereinander; eine Trennlinie liegt ausschließlich zwischen `Gesamte Sammlung` und Stapelbaum. Eingeschlossene Unterstapel bleiben markiert und deaktiviert. Gruppentöne werden dort nicht verwendet.
...
- Interaktiv angelegte oder verschobene Stapelbäume besitzen höchstens acht sichtbare Ebenen: Hauptstapel plus sieben Unterebenen. Beim direkten Drag-and-drop auf einen Stapel der achten Ebene wird ein passender gezogener Baum stattdessen als dessen Geschwister unter dem gemeinsamen Elternstapel auf Ebene 7 eingeordnet. Passt der gezogene Baum auch dort nicht in acht Ebenen, wird der Drop ohne Mutation mit `Maximal acht Stapel-Ebenen sind möglich.` abgelehnt.
- APKG-Hierarchien bleiben bis einschließlich Quell-Ebene 8 unverändert. Jeder Quellstapel ab Ebene 9 wird als eigenes, stabil identifiziertes Geschwister auf Ebene 8 unter dem Vorfahren der Quell-Ebene 7 abgelegt; sein ursprünglicher Anki-Pfad, seine Quelltiefe und sein ursprünglicher Elternpfad bleiben als Importherkunft erhalten. Karten- und Anki-Tags werden dabei nicht verändert.
...
- Oberhalb der Lernkarte steht ein beschrifteter, vollständig gefüllter Tagesfortschritt. Seine vier proportionalen Segmente zeigen von links nach rechts `Gelernt` in neutralem Grau, `Neu` im Pink von `Nochmal`, `Offen` für heute anstehendes Learning und Relearning im Orange von `Schwer` sowie `Fällig` im Gelb von `Gut`. Jede abgeschlossene Karte wechselt aus ihrer bisherigen Kategorie in `Gelernt`; ein vollständig abgeschlossener Tagesumfang ist daher ausschließlich grau. Jedes vorhandene Segment zeigt beim Hover einen einzeiligen Tooltip im Statistikstil mit farbigem Squircle, Bezeichnung und Kartenanzahl. Der sichtbare Zähler nennt heute gelernte und insgesamt heute relevante Learning Items; ein zugänglicher Text nennt zusätzlich alle vier Werte einschließlich leerer Gruppen. Nur während eines aktiven Laufs zeigt der schlanke `Pomodoro-Timer`-Balken darunter die aufgerundeten ganzen Restminuten sowie den sekündlich sinkenden Anteil.
- Die dynamische Tagesgesamtmenge vereinigt die nach aktuellen Stapel- und Tageslimits ausgewählte Queue mit den am fachlichen Lerntag bereits bearbeiteten, weiterhin reviewbaren Learning Items. Beim Sitzungsstart bildet die kanonische Stapelprojektion daraus einen stabilen Plan; Zähler und Fortschritt verwenden dessen vollständige Anzahl unabhängig davon, wie viele Kartenkörper bereits gepuffert sind. Nachladeseiten gehören zur selben Sitzung und ein Nachladefehler darf weder den Tagesumfang verkleinern noch einen Abschluss vortäuschen. Eine heute beantwortete Karte zählt als `Gelernt`, sobald an diesem Lerntag kein weiterer Schritt ansteht. Steht nach der Bewertung noch ein Learning- oder Relearning-Schritt am selben Lerntag an, wechselt sie stattdessen nach `Offen`; das gilt entsprechend aus `Neu` und `Fällig`. Eine intern noch im Zustand Learning befindliche Karte mit nächstem Schritt morgen zählt heute als `Gelernt` und erscheint am Folgetag wieder als `Offen`. Reviewkarten sind an ihrem gesamten fachlichen Fälligkeitstag verfügbar; Varianten und Wiederholungen zählen dasselbe Learning Item nicht mehrfach.
...
- Unter `Lerneinstellungen → Lerntag & Planung` stehen accountweit der Tagesbeginn von 0 bis 23 Uhr, das Vorziehfenster von 0 bis 720 Minuten und der Wochenrhythmus aus sieben Easy-Days-Stufen. Der Tagesbeginn 0 erhält Mitternacht als Grenze; bei 3 gehört 02:59:59 noch zum Vortag und um 03:00 beginnt der neue fachliche Lerntag. Das Vorziehfenster gilt für Queue und laufende Sitzung aller Stapel, ist standardmäßig 20 Minuten und wird durch 0 deaktiviert. Der Wochenrhythmus bietet je Wochentag `Normal`, `Weniger` oder `Minimal`; sieben gleiche Werte sind neutral. Diese Werte werden weder in Stapel kopiert noch von Lernprofilen verändert.
- Die unveränderlichen Vorlagen `Standard`, `Intensiv` und `Entspannt` setzen neue Karten, tägliche Wiederholungen und maximales Intervall auf `20 / 200 / 1.000`, `30 / 300 / 365` beziehungsweise `10 / 100 / 2.000`. Sie enthalten außerdem die Position neuer Karten sowie deren Sortierung nach Alter oder stabilem Lerntagszufall und die Sortierung fälliger Karten nach Überfälligkeit oder Abrufwahrscheinlichkeit. Eigene benannte Lernprofile sind konto-weite Vorlagen und werden in Lern- sowie Stapeleinstellungen verwaltet. `Auf diesen Stapel anwenden` kopiert ihre Werte und Herkunftsversion in genau einen Stapel; `Als Standardprofil verwenden` übernimmt sie in den globalen Standard. Es gibt keine Live-Vererbung. Direkte Änderungen machen Stapel beziehungsweise globalen Standard zu `Eigene Einstellungen`. Umbenennen, Aktualisieren oder bestätigtes Löschen einer Vorlage verändert keinen vorhandenen Stapel; eine neuere Inhaltsversion wird an älteren Kopien sichtbar und kann bewusst erneut angewandt werden.
- `Lerneinstellungen → Tagesrunde & Lernprofile` und `Scheduler & CoRe` definieren den vollständigen Standard für neue Stapel: Tageslimits, Kartenreihenfolge, Schedulerprofil, Varianten-Lernstufe und Anzahl aktiver Varianten. `Auf alle neuen Stapel anwenden` speichert ausschließlich diesen Standard. `Auf alle Stapel anwenden` speichert ihn ebenfalls und kopiert ihn zusätzlich in alle vorhandenen Stapel; Name, Hierarchie, Darstellung, CoRe-Modus, Karten und Lernverlauf bleiben erhalten. Ein abweichendes Neulimit hebt den Tages-Override auf. Manuelle, importierte und Demo-Stapel erhalten den Standard; Reimporte behalten die lokalen Einstellungen des bereits vorhandenen Stapels.
...
Der direkt verlinkbare `/simulator` ist unter `Lerneinstellungen → Fokuswerkzeuge` neben dem Pomodoro-Timer erreichbar. Er verschiebt die lernbezogene App-Zeit für die vorhandenen Accountkarten von „Heute“ aus minuten- oder tageweise um bis zu 3.650 Kalendertage in die Zukunft. Das Umstellen der Zeit verändert keine Karte; ein am simulierten Zeitpunkt ausgeführtes Review ist dagegen ein echtes Review und wird mit diesem Zeitpunkt gespeichert und synchronisiert.
...
- Das Zahnrad-Menü öffnet `Allgemeine Einstellungen` mit `Konto`, `Daten & Synchronisierung` und dem abschließenden Bereich `Über uns`. Die davon getrennte Seite `Lerneinstellungen` gliedert sich in `Lerntag & Planung`, `Tagesrunde & Lernprofile`, `Scheduler & CoRe` und `Fokuswerkzeuge`; Stapeleinstellungen verwenden `Stapel`, `Tagesrunde & Lernprofile` und `Scheduler & CoRe`. Die Seiten besitzen direkte Querlinks und dieselbe iconunterstützte Seiteninhaltsnavigation: Ab 1.280 px steht eine vertikale Sticky-Rail neben dem Inhalt, darunter zeigt ein Sticky-Disclosure nur den aktuellen Abschnitt und klappt alle Bereiche auf. Scrollspy markiert genau einen Abschnitt; explizite Auswahl verwendet reproduzierbare Hashlinks und Browser-Zurück. Auf Karten- und Simulatorrouten bleibt das Hauptziel `Lernen` aktiv; nur die allgemeinen Einstellungen markieren das Zahnrad als aktiv.
...
- Sobald ein allgemeines, globales Lern- oder stapelbezogenes Einstellungsfeld vom letzten gespeicherten Seitenzustand abweicht, erscheint seitenweit genau eine kompakte, nichtmodale Leiste `Änderungen speichern?` im unteren Viewportbereich. Sie hebt die noch ausstehende Speicherentscheidung und ihre Aktion mit einem dezenten CoRe-Pink-Akzent, dünnem Akzentrahmen und neutralem Tiefenschatten ab und ordnet Status und Aktion auf breiten Ansichten nebeneinander sowie mobil untereinander an. Ein zugängliches Kreuz rechts oben verwirft den gesamten offenen Seitenentwurf und stellt den zuletzt gespeicherten Stand wieder her; während des Speicherns sind Speichern und Verwerfen gesperrt. Allgemeine Einstellungen besitzen eine Speicheraktion, Lerneinstellungen die beiden Reichweiten `Auf alle Stapel anwenden` und `Auf alle neuen Stapel anwenden`, Stapeleinstellungen optional Stapel oder Stapelbaum. Erfolg schließt die Leiste und meldet die gewählte Reichweite genau einmal; Validierungs- und Speicherfehler lassen sie offen und erscheinen beim betroffenen Bereich.
...
- Tagesbeginn, Vorziehfenster, Easy Days, Lernprofile und der globale Stapelstandard sind accountweite Scheduler-Präferenzen und werden über Profil-Sync transportiert.
- Sicherheitskritische Aktionen sind klar von Profil- und Lernoptionen getrennt.

### 6.2 Deck-Hierarchie

- Decks können Eltern- und Unterstapel bilden.
- Hierarchie bleibt beim unterstützten APKG-Import bis Ebene 8 erhalten; tiefere Quellpfade werden nach dem beschriebenen Acht-Ebenen-Vertrag abgeflacht und als Importherkunft bewahrt.
...
- Stapelgruppen verwenden dauerhaft die einfachen, gerahmten Flächen `--core-group-depth-0` bis `--core-group-depth-7`: Hauptstapel verwenden die ungefüllte Oberflächenfarbe von Depth 0 ohne Schatten, die sieben Unterebenen jeweils Depth 1 bis 7. Die acht Töne interpolieren in sRGB linear zwischen der bisherigen Hauptstapelfläche und dem bisherigen tiefsten Gruppenton; eine größere technische Tiefe wird stets auf Depth 7 begrenzt. Die Skala wird im Light Mode mit zunehmender Tiefe dunkler und im Dark Mode spiegelbildlich heller. Hover füllt die vorhandene Zeilenfläche neutral; Auswahl, Fokus und Drop-Ziele reagieren weiterhin am bestehenden Außenrand, ohne einen eingerückten Layer oder eine erhöhte Stapelfläche zu erzeugen.
