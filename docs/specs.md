...
Ein Fragezeichen neben dem Theme-Schalter öffnet die direkt verlinkbare Hilfeseite `/hilfe` im normalen App-Shell-Inhaltsbereich. Ein kurzer Einstieg ordnet CoRes Ziel ein und führt Active Recall sowie Spaced Repetition als die zwei verbundenen Lernmethoden ein. Zwei anschließende Scrollgeschichten erklären erst den wechselnden Abrufreiz und danach die FSRS-Lernkurve, ohne Workspace- oder Schedulerzustand zu verändern.
...
- Die Active-Recall-Geschichte besitzt drei aufeinanderfolgende Textschritte: einen Kartenstapel, das passive Wiedererkennen einer verblassenden Kartenform und mehrere nahe Fragen mit derselben Antwort. Ab Desktopbreite bleibt die Kartenvisualisierung beim Scrollen innerhalb dieses Abschnitts stehen und wechselt passend zum aktiven Schritt; darunter bleibt die Abfolge linear.
