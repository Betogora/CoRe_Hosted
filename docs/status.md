...
- Review mit vier Bewertungen, echtem FSRS-6 auf offiziellen Standardparametern, Tastatursteuerung und deterministischer Intervallvorschau. Die unveränderte `ts-fsrs`-Strategie lässt `Gut` die Lernschritte durchlaufen und `Leicht` sofort graduieren. Der accountweite, von 0 bis 23 Stunden nach Mitternacht einstellbare Tagesbeginn wird DST-sicher in der Profilzeitzone ausgewertet; Reviewkarten sind an ihrem gesamten fachlichen Fälligkeitstag verfügbar. Sitzungen arbeiten eindeutige Karten vor berechtigten Wiederholungen ab; das accountweite, von 0 bis 720 Minuten einstellbare Vorziehfenster gilt standardmäßig 20 Minuten für alle Stapel, niemals für Reviewkarten und niemals über die fachliche Tagesgrenze. Heute beantwortete Karten ohne weiteren Schritt am selben Lerntag zählen als `Heute geschafft`; wartet nur noch ein späterer Tagesschritt, endet die Runde mit `Für jetzt geschafft`. Aussetzen bewahrt Zustand und Termin unverändert und schließt das Learning Item aus Queue und Tagesprojektionen aus. Die Antwortzeit wird pro sichtbarer Karte monoton gemessen, bei 60 Sekunden gedeckelt und als optionales `ReviewEvent.responseTimeMs` gespeichert.
...
- Globale Einstellungen und Stapeleinstellungen besitzen je drei responsive Bereichskarten und direkte Quernavigation. Global liegen nur Konto/Datenschutz, Tagesbeginn, Vorziehfenster, Fokuswerkzeuge und Daten/Sync; Tageslimits, Scheduler und CoRe bleiben stapelspezifisch. Die decklose Stapeleinstellungsroute bietet eine echte Stapelauswahl.
- Standard, Intensiv und Entspannt sind unveränderliche Lernprofil-Vorlagen. Eigene konto-weite Vorlagen werden ausschließlich in den Stapeleinstellungen angelegt, umbenannt, versioniert, aktualisiert und bestätigt gelöscht. Anwenden kopiert Werte in genau einen Stapel; spätere Profiländerungen und globale Saves verändern keine bestehenden Stapel.
...
- APKG-Importe übernehmen zuordenbare `revlog`-Zeilen deterministisch und duplikatfrei als optionale Analysehistorie. Diese Ereignisse ändern niemals Review State, Fälligkeit oder FSRS-Planung; übersprungene, beschädigte, manuelle und nicht zuordenbare Zeilen erscheinen im Importbericht.
