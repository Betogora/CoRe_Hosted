...
- Review mit vier Bewertungen, echtem FSRS-6 auf offiziellen Standardparametern, Tastatursteuerung und deterministischer Intervallvorschau. Die unveränderte `ts-fsrs`-Strategie lässt `Gut` die Lernschritte durchlaufen und `Leicht` sofort graduieren. Sitzungen arbeiten eindeutige Karten vor berechtigten Wiederholungen ab; das stapelbezogene, von 0 bis 720 Minuten einstellbare Vorziehfenster gilt standardmäßig 20 Minuten, niemals für Reviewkarten und niemals über die lokale beziehungsweise simulierte Tagesgrenze. Heute beantwortete Karten ohne weiteren Schritt am selben Lerntag zählen als `Heute geschafft`; wartet nur noch ein späterer Tagesschritt, endet die Runde mit `Für jetzt geschafft`. Aussetzen bewahrt Zustand und Termin unverändert und schließt das Learning Item aus Queue und Tagesprojektionen aus. Die Antwortzeit wird pro sichtbarer Karte monoton gemessen, bei 60 Sekunden gedeckelt und als optionales `ReviewEvent.responseTimeMs` gespeichert.
