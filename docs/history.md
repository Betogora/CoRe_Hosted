...
- Der atomare Replica-Commit `39e879e` wurde auf `codex/prerelease-replica-v2` gepusht; das automatische Vercel-Preview ist `READY`. Sein Bundle verweist noch auf `hirbiuiydczmnjqtoyqx.supabase.co`, während lokal keine Projekt-Ref verknüpft ist und der verbundene Supabase-Account nur ein anderes Projekt zeigt. Der destruktive Remote-Reset und das strengere Replica-RPC-Ziel von 1.000 ms p95 bleiben deshalb bis zur eindeutigen Zielbestätigung und grünen Hosted-Queryplänen ausstehend. Es wurde keine Remote-Datenbank verändert.
