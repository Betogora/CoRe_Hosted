...
`AppNavigation` besitzt die einzige responsive Shell-Navigation: CSS schaltet unter 1280 px auf kompakten Kopf und schwebende Bottom Bar, ab 1280 px auf die Sidebar. Die mobile Leiste wird per Portal außerhalb des weichgezeichneten Shell-Containers gerendert und verwendet dynamische Viewportmaße, damit lange Seiten und Browser-Scrollbars ihre feste Position oder Breite nicht verändern. Beide Projektionen rufen denselben typisierten Navigationscallback auf; es gibt keine Geräteerkennung oder zweite Routebene. `Mehr` ist die mobile Bezeichnung der direkt geöffneten Kartenverwaltung. Eine private, aus gemeinsamen `IconButton`-Primitives gebaute Utility-Gruppe projiziert in beiden Layouts das umrundete Einstellungszahnrad und rechts daneben den randlosen Theme-Button. Einstellungen, Hilfe und Simulator teilen weiterhin den aktiven Einstellungszustand; Profilinitialen und Anzeigename gehören nicht zur Navigation. Ein aktiver Pomodoro-Timer erscheint im mobilen Kopf zwischen Marke und Utility-Gruppe beziehungsweise in der Sidebar unmittelbar oberhalb ihrer Trennlinie. Fachliche Komponenten koppeln ihre interne Dichte nicht an diesen Viewport-Breakpoint, sondern an ihre verfügbare Containerbreite.
