...
| `src/studyHeatmapModel.ts` | Gemeinsames dünnes Tageszählermodell und responsive Wochenfenster-Projektion für Dashboard und Statistik |
...
`StatisticsScreen` bleibt lazy geladen und kennt nur die aggregierten Ergebnisse aus `src/statisticsModel.ts`. Das Modell indexiert Review Events und aktuelle Karten-/Varianten-Snapshots einmal je Deckreferenz. Pro Index hält es höchstens eine ersetzbare, zeitzonen- und stapelbereichsbezogene Ableitung für Kartenanlage, wahre Erinnerungsquote und kompakte Tag-/Stundenindizes; sie wächst nicht mit Filterwechseln und wird mit dem Index freigegeben. Zeiträume werden je sortierter Stapelreihe über binäre Grenzen ausgeschnitten und anschließend in einem gemeinsamen Durchlauf aggregiert. Diagramme erhalten höchstens 240 voraggregierte Punkte. Die Statistik-Heatmap speichert nur Tage mit Reviews sowie ihre Bereichsgrenzen; `StudyHeatmap` materialisiert daraus ausschließlich das responsive sichtbare Wochenfenster und wird auch vom Dashboard verwendet. Der lokale strukturierte Statistiktooltip verbreitert den kurzen produktweiten `CoreTooltip` nicht.
