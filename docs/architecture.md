...
`StudySettingsOverlay` besitzt die gemeinsame responsive Projektion der Lerneinstellungen: ein Portal rendert unter 768 px ein Bottom Sheet und ab 768 px einen zentrierten modalen Dialog aus demselben DOM. Backdrop, Viewportbegrenzung, interner Scrollbereich, Fokusfalle, Escape-/Außenklick-Schließen, Fokuswiederherstellung und Reduced Motion gehören dem UI-Modul. `CoreSwitch` ist die kontrollierte gemeinsame Switch-Basis für Theme, Lerneinstellungen und Kartendetail; `CardMarkButton` besitzt die gemeinsame `aria-pressed`-Sternsemantik. `StudyMode` liefert aktuelle Kartenidentität, Stapelgrenzwerte und typisierte Aktionen. Markierung liegt in `LearningItem.meta.marked`, Aussetzung im bestehenden `LearningItem.status`; die zentrale Workspace-Mutation erhöht Revision und Versionslog und läuft durch die State-Outbox. Eine Aussetzung entfernt die Karte zusätzlich aus der lokalen Sitzung, während Scheduler-, Review- und Persistenzlogik in ihren fachlichen Modulen verbleiben.
