...
- Normalisierte `DeckSettings` enthalten die aktiven Lernfelder `newCardsPerDay`, `maximumReviewsPerDay`, `newReviewOrder` und `schedulerProfile`, die CoRe-Felder `coreMode`, `variantThresholdXp`, `maxActiveVariantsPerCard` sowie die technischen Ausschlusslisten `blacklist.cardTypes` und `blacklist.tags`. Die ungenutzten Felder `aiPolicy` und `blacklist.variantIds` sind kein Teil des kanonischen Vertrags und werden beim Normalisieren verworfen.
...
Die Tagesqueue begrenzt nur eindeutige initiale Karten und verrechnet gegen beide Tagesgrenzen bereits heute verbrauchte eindeutige Karten; Mehrfachkontakte derselben Karte zählen nicht erneut. Der Sitzungszustand hält Wiederholungen separat und zieht sie nach der Anfangsqueue bei Bedarf vor; jede Bewertung bleibt trotzdem genau ein unveränderliches Review Event. Werden Grenzwerte während einer Sitzung geändert, ersetzt die Queue nur noch nicht begonnene initiale Einträge durch die neu berechnete Auswahl. Abgeschlossene Karten, Bewertungszähler und vorgemerkte Pflicht-Wiederholungen bleiben erhalten; ein Grenzwert unter dem bereits verbrauchten Umfang ergibt ein Restbudget von null, ohne den gespeicherten Wert zurückzusetzen.
