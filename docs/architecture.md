...
`LearningSettingsPanel` besitzt die gemeinsame kompakte Feldprojektion für globale und stapelspezifische Lernoptionen. `Neue Karten pro Tag`, `Wiederholungen pro Tag` und `Maximales Intervall in Tagen` verwenden ein lokales responsives Ganzzahlfeld, das Eingaben erst beim Verlassen oder mit Enter innerhalb der fachlichen Grenzen übernimmt; nur die beiden erstgenannten Felder stehen im Tagespensum-Raster. Selbsterklärende Titel erhalten keinen wiederholenden Hinweis; fachliche Folgen, Abwägungen und Warnungen bleiben sichtbar. Der globale Aufrufer aktiviert automatisches Speichern. `Profile.schedulerPreferences.profile` hält dabei das aktive Profil getrennt von `Profile.schedulerPreferences.deckSettings`, das den letzten eigenen Entwurf bewahrt; dadurch überlebt dieser Entwurf Preset-Wechsel und Neuladen. Stapelspezifische Aufrufer speichern weiterhin ausdrücklich. `DeckSettingsScreen` ergänzt ausschließlich für Stapel `variantThresholdXp` und `maxActiveVariantsPerCard` und trennt die Speicherkommandos: Die Kopf-Toolbar schreibt nur Name und Darstellung, `Lernoptionen speichern` schreibt Lernwerte, CoRe-Modus und beide CoRe-Parameter. Presets mutieren diese beiden Parameter nicht. `deckSettings.ts` löst die drei Zahlenwerte benannter Presets kanonisch auf, während `custom` gespeicherte Werte beibehält und bei fehlenden Werten die Standardbasis `20 / 200 / 1.000` verwendet. `learnAheadMinutes` ist Teil der normalisierten Lernwerte, wird als Ganzzahl von 0 bis 720 im vorhandenen `deck_settings`-JSONB gespeichert und erhält für ältere Payloads sowie alle Presets den Standardwert 20. Tagespensum-Felder verwenden breite Vier-, mittlere Zwei- und schmale Einspaltenprojektionen; die drei Content-Repetition-Felder verwenden entsprechend drei, zwei oder eine Spalte. Beide Bereiche erzeugen kein horizontales Hauptscrolling.
...
`StatisticsScreen` bleibt lazy geladen und kennt nur die aggregierten Ergebnisse aus `src/statisticsModel.ts`. Das Modell indexiert Review Events und aktuelle Karten-/Varianten-Snapshots einmal je Deckreferenz. Pro Index hält es höchstens eine ersetzbare, zeitzonen- und stapelbereichsbezogene Ableitung für Kartenanlage, wahre Erinnerungsquote, allzeitliche dünne Heatmap-Tageszähler und kompakte Tag-/Stundenindizes; sie wächst nicht mit Filterwechseln und wird mit dem Index freigegeben. Globale Statistikzeiträume werden je sortierter Stapelreihe über binäre Grenzen ausgeschnitten und anschließend in einem gemeinsamen Durchlauf aggregiert. Diagramme erhalten höchstens 240 voraggregierte Punkte. Die Heatmap bleibt davon unabhängig, folgt aber demselben Stapel-Scope. `studyHeatmapModel.ts` berechnet daraus den aktuellen Streak, die getrennten historischen und prognostizierten Intensitäten sowie exakte Kalenderfenster von der ersten Aktivität bis zum vollständigen Zeitraum mit Tag +365. Die Prognose zählt pro aktivem Learning Item nur den kanonischen nächsten `learningItemState`-/`reviewState`-Termin in der Profilzeitzone; Varianten und blockierte Items werden nicht verdoppelt oder simuliert. `StudyHeatmap` besitzt die lokale Auswahl `Woche`, `Monat`, `Jahr`, die beidseitige Zeitraum-Navigation, den unveränderten historischen Lila-Verlauf, eine dezente semantische Grauskala für Prognosetage, einen ab 34 rem eigenen Breite einzeiligen und darunter stabil zweizeiligen Kopf sowie das auf schmalen Ansichten horizontal scrollbare vollständige Jahresraster und wird auch vom Dashboard verwendet. Der lokale strukturierte Statistiktooltip verbreitert den kurzen produktweiten `CoreTooltip` nicht.
