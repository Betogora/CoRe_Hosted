...
`StatisticsScreen` bleibt lazy geladen und kennt nur die aggregierten Ergebnisse aus `src/statisticsModel.ts`. Das Modell indexiert Review Events und aktuelle Karten-/Varianten-Snapshots einmal je Deckreferenz. Pro Index hält es höchstens eine ersetzbare, zeitzonen- und stapelbereichsbezogene Ableitung für Kartenanlage, wahre Erinnerungsquote, allzeitliche dünne Heatmap-Tageszähler und kompakte Tag-/Stundenindizes; sie wächst nicht mit Filterwechseln und wird mit dem Index freigegeben. Globale Statistikzeiträume werden je sortierter Stapelreihe über binäre Grenzen ausgeschnitten und anschließend in einem gemeinsamen Durchlauf aggregiert. Diagramme erhalten höchstens 240 voraggregierte Punkte. Die Heatmap bleibt davon unabhängig, folgt aber demselben Stapel-Scope. `studyHeatmapModel.ts` berechnet daraus den aktuellen Streak und exakte Kalenderfenster; `StudyHeatmap` besitzt die lokale Auswahl `Woche`, `Monat`, `Jahr`, die Zeitraum-Navigation, einen ab 34 rem eigenen Breite einzeiligen und darunter stabil zweizeiligen Kopf sowie das auf schmalen Ansichten horizontal scrollbare vollständige Jahresraster und wird auch vom Dashboard verwendet. Der lokale strukturierte Statistiktooltip verbreitert den kurzen produktweiten `CoreTooltip` nicht.
