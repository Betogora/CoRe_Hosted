...
`StudySettingsOverlay` besitzt die gemeinsame responsive Projektion der Lerneinstellungen: ein Portal rendert unter 768 px ein Bottom Sheet und ab 768 px einen zentrierten modalen Dialog aus demselben DOM. Backdrop, Viewportbegrenzung, interner Scrollbereich, Fokusfalle, Escape-/Außenklick-Schließen, Fokuswiederherstellung und Reduced Motion gehören dem UI-Modul. `StudyMode` liefert aktuelle Kartenidentität, Stapelgrenzwerte und typisierte Aktionen; Queue- und Persistenzlogik verbleiben in ihren fachlichen Modulen.
