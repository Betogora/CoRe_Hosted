...
`SettingsSectionNavigation` besitzt die gemeinsame responsive Sprungnavigation beider Einstellungsseiten: drei semantisch gefärbte, per Tastatur aktivierbare Bereichskarten stehen ab mittlerer Breite nebeneinander und mobil untereinander. `SettingsCrossLinkButton` ist der einzige kompakte Querlinkstil. `LearningSettingsPanel` gehört ausschließlich den Stapeleinstellungen und projiziert Tagespensum, Lernprofil-Vorlagen, Scheduler und CoRe in zwei getrennte Bereiche. Der Bereich `Stapel` besitzt Name, Icon, Farbe und Verwaltungsaktionen; die breite Darstellungs-Toolbar ist kein Teil des Seitenkopfs mehr.
