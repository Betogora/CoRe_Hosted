...
`LearningSettingsPanel` besitzt die gemeinsame kompakte Feldprojektion für globale und stapelspezifische Lernoptionen. `Neue Karten pro Tag`, `Wiederholungen pro Tag` und `Maximales Intervall in Tagen` verwenden ein lokales responsives Ganzzahlfeld, das Eingaben erst beim Verlassen oder mit Enter innerhalb der fachlichen Grenzen übernimmt; nur die beiden erstgenannten Felder stehen im Tagespensum-Raster. Selbsterklärende Titel erhalten keinen wiederholenden Hinweis; fachliche Folgen, Abwägungen und Warnungen bleiben sichtbar. Der globale Aufrufer aktiviert automatisches Speichern und ergänzt `dayStartHour` als natives Ganzzahlfeld von 0 bis 23 mit außen stehender Einheit; der Stapelaufrufer erhält dieses Feld nicht und speichert weiterhin ausdrücklich. `Profile.schedulerPreferences.profile` hält das aktive Lernprofil getrennt von `Profile.schedulerPreferences.deckSettings`, das den letzten eigenen Entwurf bewahrt; dadurch überlebt dieser Entwurf Preset-Wechsel und Neuladen. `DeckSettingsScreen` ergänzt ausschließlich für Stapel `variantThresholdXp` und `maxActiveVariantsPerCard` und trennt die Speicherkommandos: Die Kopf-Toolbar schreibt nur Name und Darstellung, `Lernoptionen speichern` schreibt Lernwerte, CoRe-Modus und beide CoRe-Parameter. Presets mutieren diese drei global beziehungsweise stapelspezifisch ergänzten Parameter nicht. `deckSettings.ts` hält `GlobalLearningSettings` von `LearningSettings` und `DeckSettings` getrennt, normalisiert `dayStartHour` unter `Profile.schedulerPreferences` mit Standard 0 und löst die drei Zahlenwerte benannter Presets kanonisch auf, während `custom` gespeicherte Werte beibehält und bei fehlenden Werten die Standardbasis `20 / 200 / 1.000` verwendet. `learnAheadMinutes` ist Teil der normalisierten stapelbezogenen Lernwerte, wird als Ganzzahl von 0 bis 720 im vorhandenen `deck_settings`-JSONB gespeichert und erhält für ältere Payloads sowie alle Presets den Standardwert 20. Tagespensum-Felder verwenden breite Vier-, mittlere Zwei- und schmale Einspaltenprojektionen; die drei Content-Repetition-Felder verwenden entsprechend drei, zwei oder eine Spalte. Beide Bereiche erzeugen kein horizontales Hauptscrolling.
...
- Normalisierte `DeckSettings` enthalten die aktiven Lernfelder `newCardsPerDay`, `maximumReviewsPerDay`, `learnAheadMinutes`, `newReviewOrder` und `schedulerProfile`, die CoRe-Felder `coreMode`, `variantThresholdXp`, `maxActiveVariantsPerCard` sowie die technischen Ausschlusslisten `blacklist.cardTypes` und `blacklist.tags`. Die ungenutzten Felder `aiPolicy` und `blacklist.variantIds` sind kein Teil des kanonischen Vertrags und werden beim Normalisieren verworfen.
- Die drei Scheduler-Anfangsintervalle bleiben aus Kompatibilitätsgründen normalisiert und persistierbar, werden aber nicht als wirksame Einstellungen angeboten.
...
Die Tagesqueue projiziert New, Learning/Relearning und Review getrennt. New und Review werden gegen die verbleibenden Tagesgrenzen ausgewählt; heute anstehendes Learning/Relearning bleibt davon unabhängig `In Arbeit`. Elternsitzungen verwenden für den gesamten Teilbaum die Tagesgrenzen und `learnAheadMinutes` des gestarteten Elternstapels. Reviewkarten werden für ihren gesamten fachlichen Fälligkeitstag freigegeben und nie vorgezogen. Learning/Relearning bleibt minutengenau bei erreichtem Termin oder strikt innerhalb des Fensters verfügbar, sofern der Termin am selben fachlichen Lerntag liegt. Scheduler-Vorschau und Commit erhalten denselben Lerntagskontext und persistieren dadurch denselben `learningDayKey`; FSRS-Formeln und Fälligkeitszeitstempel bleiben unverändert. Der Sitzungszustand hält Wiederholungen separat, arbeitet Initialkarten zuerst ab und durchsucht Wiederholungen FIFO nach dem ersten berechtigten Eintrag, damit ein noch nicht berechtigter Termin spätere berechtigte Wiederholungen nicht blockiert. `DailyReviewProgressSummary.completedTodayCount` zählt heute beantwortete Karten ohne weiteren Schritt an diesem Lerntag; ein nächster Learning-Schritt am Folgetag bleibt intern Learning und wird an diesem Folgetag wieder `In Arbeit`. Jede Bewertung bleibt genau ein unveränderliches Review Event. Werden Grenzwerte während einer Sitzung geändert, ersetzt die Queue nur noch nicht begonnene initiale Einträge durch die neu berechnete Auswahl. Abgeschlossene Karten, Bewertungszähler und vorgemerkte Wiederholungen bleiben erhalten; ein Grenzwert unter dem bereits verbrauchten Umfang ergibt ein Restbudget von null, ohne den gespeicherten Wert zurückzusetzen.

## 6. Persistenz, Sync und Medien

- `src/accountStorage.ts` trennt lokale Cache-Keys pro Account.
- `Profile.schedulerPreferences.dayStartHour` transportiert die accountweite, auf 0 bis 23 normalisierte Startstunde durch den bestehenden Profil-Sync sowie Workspace-Export und -Import. `deck_settings` enthält keine Kopie und benötigt keine Migration.
